...
def _packed_uniform_matrix(carrier: Carrier) -> np.ndarray:
    """Evaluate the packed uniform row in bounded target/source tiles."""
    target_tile = 24
    source_tile = 24
    result = np.empty((len(carrier.node), len(carrier.polygons)), dtype=np.float64)
    for target_start in range(0, len(carrier.node), target_tile):
        ...
        for source_start in range(0, len(carrier.polygons), source_tile):
            source_stop = min(source_start + source_tile, len(carrier.polygons))
            sections = carrier.polygons[source_start:source_stop]
...
def _measure_carrier(case, carrier: Carrier) -> tuple[dict[str, Any], np.ndarray]:
    ...
    closed = np.zeros((len(target_r), len(carrier.polygons)), dtype=np.float64)
    reference = np.zeros_like(closed)
    reference_check = np.zeros_like(closed)
    per_source_rms = np.zeros(len(carrier.polygons), dtype=np.float64)
    per_source_aspect = np.zeros(len(carrier.polygons), dtype=np.float64)
    edge_absolute = []
    edge_reference = []
    edge_check = []
    live_edge_count = 0

    for source, vertices in enumerate(carrier.polygons):
    ...
    packed = _packed_uniform_matrix(carrier)
    packed_scalar_delta = packed - closed
    current = (
        np.asarray(
            case.toroidal_current_density(carrier.node[:, 0], carrier.node[:, 1]),
            dtype=np.float64,
        )
        * carrier.area
    )
