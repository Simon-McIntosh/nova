#!/bin/bash
# One allocation, two fresh pytest processes with output capture off, so text a
# native library writes straight to file descriptor 2 reaches the log.
...
arm() {  # name extra_env targets...
  ...
  log=$OUT/$name.log
  printf 'revision=%s tree=%s host=%s command=env %s %s -m pytest -p no:cacheprovider -vv --tb=short --capture=no %s\n' \
    "$REV" "$TREE" "$(hostname)" "$extra" "$PY" "$*" > "$log"
  (env $extra "$PY" -m pytest -p no:cacheprovider -vv --tb=short --capture=no "$@" >> "$log" 2>&1; \
   printf 'EXIT=%s\n' "$?" >> "$log") &
  wrapper=$!
  sleep 2
...
printf 'revision=%s host=%s start=%s max_map_count=%s nproc=%s\n' "$REV" "$(hostname)" "$(date -Is)" \
  "$(cat /proc/sys/vm/max_map_count)" "$(nproc)" > "$OUT/allocation-uncaptured.txt"
printf 'end=%s\n' "$(date -Is)" >> "$OUT/allocation-uncaptured.txt"
arm uncaptured-abort-alone JAX_ENABLE_COMPILATION_CACHE=true $ABORT &
arm uncaptured-certificate-then-abort JAX_ENABLE_COMPILATION_CACHE=true $FIRST $ABORT &
wait
