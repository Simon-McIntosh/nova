#!/bin/bash
# One H200 allocation: the width-16 vmapped cost of the batching rule and of
...
L=$D/batch-rule-cost-h200.log
echo "revision=$(git rev-parse HEAD) tree=$W command=$PY vmap_exit_cost.py rule job=${SLURM_JOB_ID:-none}" > $L
"$PY" $D/vmap_exit_cost.py $D/batch-rule-cost-h200.json rule >> $L 2>&1; echo "EXIT=$?" >> $L
L=$D/batch-rule-negative-control-h200.log
...
"$PY" $D/vmap_exit_cost.py $D/batch-rule-negative-control-h200.json selected >> $L 2>&1; echo "EXIT=$?" >> $L
