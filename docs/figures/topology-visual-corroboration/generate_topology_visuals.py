...
def _draw_row(row: dict[str, Any], path: Path) -> dict[str, Any]:
    ...
    if len(polygons):
        ...
        axis.autoscale_view()
        legend_names = {
            int(PlasmaDomain.EXCLUDED_MATERIAL): "flood fill: excluded material",
            int(PlasmaDomain.CORE): "flood fill: axis-connected core",
            int(PlasmaDomain.COMMON_SOL): "flood fill: common SOL",
            int(PlasmaDomain.PRIVATE_FLUX): "flood fill: private exclusion",
        }
    ...
    axis.text(
        0.01,
        0.01,
        "FLUX raster · colours are connectivity components, not cell types · "
        "six-neighbour index adjacency",
    ...
    axis.legend(
        unique.values(),
        unique.keys(),
        title="Flux-grid connectivity and landmarks",
        loc="upper center",
...
def _write_evidence(rows: list[dict[str, Any]], records: list[dict[str, Any]]) -> None:
  ...
  <figure><img src="/nova/figures/topology-visual-corroboration/{filename}" alt="Topology evidence for {row["machine"]} {row["identity"]}: rectangular flux-raster cells coloured by flood-fill connectivity component, production six-neighbour adjacency segments, all Nova O and X candidates, selected primary O and X, wall point, and EFIT axis, X labels and LCFS overlay."><figcaption>{record["voronoi_cells"]} rectangular flux-raster cells; {record["wall_admissible_adjacency_links"]} wall-admissible six-neighbour adjacency segments; {record["private_flux_cells"]} cells in the private-exclusion connectivity component; {record["o_candidates"]} plotted O candidates; {record["x_candidates"]} plotted X candidates; selected O/X/wall markers {record["selected_o"]}/{record["selected_x"]}/{record["wall_point"]}; EFIT axis/X/LCFS vertices {record["efit_axis"]}/{record["efit_x"]}/{record["efit_lcfs_vertices"]}. <strong>Containment record:</strong> {containment}. This raster is the tensor-product <em>flux grid</em>; its colours are flood-fill connectivity components, never current-cell types. The solve consumes the resulting private mask and evaluates flux-function continuations on the applicable open domains. The production half-offset stencil supplies six-neighbour hex connectivity as an index interpretation, drawn here from its physical shared-edge segments. The centre-in-polygon wall test admits or excludes each whole rectangle; the solver computes neither partial clipping nor polygon-wall intersections, so excluded cells that straddle the wall remain visible at full extent. Production centre/label wall-check mismatches: {record["wall_membership_mismatch_cells"]}. <strong>{status}</strong></figcaption></figure>
</article>"""
        )
    head = subprocess.run(
        ["git", "rev-parse", "HEAD"],
...
<p class="lede">All {EXPECTED_MAST_ROWS} MAST arms and all {EXPECTED_DIIID_ROWS} DIII-D demonstration frames are shown exactly once. Every populated panel exposes the rectangular tensor-product flux raster coloured by flood-fill connectivity component, the production six-neighbour adjacency used to interpret that raster, the complete finite O/X candidate census, selected primary O and X, selected wall point, and EFIT axis/X/LCFS labels.</p></header>
...
<p><strong>The coloured raster is a connectivity partition, not a catalogue of current-cell types.</strong> Purple is the exact <code>PRIVATE_FLUX</code> component: closed-flux raster nodes disconnected from the primary O-point by the X-point flood-fill cut. Pale blue is the axis-connected component, grey-green is the common-SOL component, and pale grey is excluded material. The solve consumes this partition as a private mask and as the selector for continuation domains. It still resolves current by evaluating the pressure and field-function gradients at each cell's normalised flux; outside the closed boundary, <code>OPEN_DOMAINS</code> and <code>gradient_tail</code> supply the declared continuations of p-prime and FF-prime. It never consumes a per-cell “type”. The selected wall marker is Nova's governed closest plasma-wall candidate; it is shown even when the topology class is diverted and it does not bind the LCFS.</p>
<p><strong>The flux raster and plasma-current mesh are different grids.</strong> The panels can reproduce the former exactly from the committed operands. Nova's production current mesh is a separate, half-offset tiling of genuine hexagonal cells constructed by <code>PolyGrid</code>; its material polygons are atomised by <code>AtomicCellMesh.from_cells</code>, and <code>AtomicCellMesh.traced_clip</code> traces the separatrix through those polygons from signed flux at the shared atomic nodes. The committed topology caches contain neither the production <code>current_cell_polygons</code> nor <code>atomic_node_signed_flux</code>. A truthful traced-clip layer therefore cannot be reconstructed here: selecting an unstored tiling resolution or deriving node signs from the plotted boundary would manufacture geometry or flux. Those two fields must be serialized during the next bank regeneration before the current-mesh layer is published.</p></section>
...
<section><h2>Deferred plasma-current mesh layer</h2><p>The genuine current mesh cannot be substituted by the flux raster. Publication requires the exact production <code>current_cell_polygons</code> and the <code>atomic_node_signed_flux</code> consumed by <code>AtomicCellMesh.traced_clip</code>. Neither is present in these committed operand caches. Bank both fields alongside the topology operands during the next scheduled extraction regeneration; then the traced core/common support polygons can be rendered without a solve or a geometric inference.</p></section>
