...
class extract_geom(object):
    ...
    def set_current(self, name, I):
        self.coil[name]['I'] = I
        self.coil[name]['rc'] = (
            abs(self.coil[name]['I']) / (self.Jmax * np.pi))**0.5
    ...
    def set_plasma(self):
        ...
        for i in range(self.N - 1):
            r = np.array([self.rp[i + 1] - self.rp[i],
                          self.zp[i + 1] - self.zp[i], 0])
        ...
        self.theta = np.unwrap(np.arctan2(
            self.zp - self.zc, self.rp - self.rc))
    ...
    def elipsoid(self, zo, ro, A, k, tau):
        R = ro + ro / A * np.cos(self.theta_elip +
                                 tau * np.sin(self.theta_elip))
        ...
        if grid is 'elip':  # elipsoid
            ...
            for shift, r in zip(rshift, rspace):
                if r == 0.0:
                    ...
                    L = np.cumsum(
                        np.sqrt(np.gradient(R)**2 + np.gradient(Z)**2))
            ...
            Ci = (1 - (1 - f) * (np.arange(N) / (N - 1))**2) * \
                nc  # I = Io*sum(Ci) = Io*sum(nci(1-xi/(N-1)))
            Io = I / np.sum(Ci)
