...
for i, folder in enumerate(folders['name'][index]):
    ...
    for j, name in enumerate(scn.IcCS):
        ax[i].plot(scn.t, scn.post['DINA']['Ic'][name], '-',
                   color=f'C{j%10}', label=name)
    scn.get_max_value(scn.t, scn.post['DINA']['Ic'],
                      ax=ax[i], plot=True, unit='kA')
    scn.get_max_value(scn.t, scn.post['DINA']['Ic'],
                      ax=ax[i], plot=True, unit='kA', sign=-1)
    ax[i].text(1, 0.15, folder, transform=ax[i].transAxes,
