...
class ClusterTurns:
    ...
    def extract_clusters(self):
        ...
        self.clusters = sklearn.cluster.KMeans(
            n_clusters=self.n_clusters, random_state=6, n_init=20, tol=1e-4
        ).fit_predict(self.turns)
    ...
    def build_mesh(self):
        ...
        for i in range(18):
            ...
            point_data = {
                name: coil[name].reshape(134, -1, 3)
                for name in coil.point_data
                if len(coil[name].shape) == 2
            }
    ...
    def plot_slice(self, axes=None, ms=8):
        ...
        for i in range(self.n_clusters):
            ...
            axes.plot(
                self.turns[index, 0].mean(),
                self.turns[index, 1].mean(),
                ".",
                color="gray",
            )
            axes.axis("equal")
