...
class Convolve:
    ...
    def interpolate(self, waveform):
        ...
        return np.fft.irfft(np.fft.rfft(waveform), n=self.ndiv) * self.ndiv / self.ncoil
    ...
    def make_frame(self, t):
        ...
        convolution = np.convolve(
            np.r_[self.data.gap, self.data.gap], self.spatial_filter, mode="valid"
        )
        ...
        plt.bar(
            self.data.index - self.data.ncoil,
            self.data.gap,
            color="gray",
            zorder=-2,
            width=0.9,
        ...
        plt.bar(
            self.data.index - self.data.ncoil + index,
            self.spatial_filter[::-1],
            color="C3",
            zorder=0,
            label="filter",
            width=0.9,
