...
class FieldData(BaseData, ModelData):
    ...
    def build(self, ndiv=360):
        ...
        self.data["sample_field"] = (
            ("coordinate", "sample_phi"),
            np.zeros((self.data.sizes["coordinate"], self.data.sizes["sample_phi"])),
        )
        self.data["phi"] = np.linspace(0, 2 * np.pi, ndiv)
        self.data["field"] = (
            ("coordinate", "phi"),
            np.zeros((self.data.sizes["coordinate"], self.data.sizes["phi"])),
        )
        ...
        for i, coordinate in enumerate(self.data.coordinate.values):
            ...
            self.data.field[i] = self._filter_data(self.data.sample_field[i], False)
            self.data.ripple_field[i] = self._filter_data(
                self.data.sample_field[i], True
            )
        return self.store()
    ...
    def plot(self):
        ...
        for i, coordinate in enumerate(self.data.coordinate.values):
            ...
            if coordinate == "phi":
                coordinate = r"\phi"
            axes[i].set_ylabel(rf"$B_{coordinate}$")
        plt.despine()
...
class DeviationData(BaseData):
    ...
    def build(self, ndiv=360):
        ...
        self.data["sample_deviation"] = (
            ("simulation", "sample_phi"),
            np.zeros((self.data.sizes["simulation"], 2 * ndiv)),
        )
        self.data["phi"] = np.linspace(0, 2 * np.pi, ndiv, endpoint=False)
        self.data["deviation"] = (
            ("simulation", "phi"),
            np.zeros((self.data.sizes["simulation"], ndiv)),
        )
        self.data["ripple_deviation"] = (
            ("simulation", "phi"),
            np.zeros((self.data.sizes["simulation"], ndiv)),
        )
        for i, simulation in enumerate(self.simulations):
            self.data.sample_deviation[i] = self._load_data(simulation)
            self.data.deviation[i] = self._filter_data(
                self.data.sample_deviation[i], False
            )
            self.data.ripple_deviation[i] = self._filter_data(
                self.data.sample_deviation[i], True
            )
        self.data["peaktopeak"] = self.data.deviation.max(
            axis=-1
    ...
    def plot(self, simulation: str):
        """Plot benchmark data."""
        axes = plt.subplots(2, 1, gridspec_kw=dict(height_ratios=[1, 2], hspace=0.3))[1]
        axes[0].bar(
            self.data.index,
            self.data.delta.sel(simulation=simulation, response="radial"),
            label="radial",
        )
        axes[0].bar(
            self.data.index,
            self.data.delta.sel(simulation=simulation, response="tangential"),
            label="tangential",
            width=0.4,
        )
        ...
        axes[0].legend(
            fontsize="xx-small", ncol=2, loc="upper right", bbox_to_anchor=(1, 1.3)
        )
...
class Ansys(DeviationData, ModelData):
    ...
    def build(self):
        ...
        super().build()
        for i, simulation in enumerate(self.simulations):
            self.data.delta[i] = structural.Transform(simulation).data.response_delta
        return self.store()
