...
class DiagramBase:
    ...
    def add_node(self, name, label, shape, **kwargs):
        """Add node to graph."""
        self.graph.add_node(pydot.Node(name, label=label, shape=shape, **kwargs))
...
class Filter(DiagramBase):
    ...
    def __post_init__(self):
        ...
        super().__post_init__()
        self.add_node(
            "signal", "signal", shape="rectangle", color="darkblue", penwidth=2.5
        )
        self.add_node(
            "filter", "filter", shape="rectangle", color="darkred", penwidth=2.5
        )
        self.add_node(
            "response", "response", shape="rectangle", color="darkgreen", penwidth=2
        )
...
class FFT(DiagramBase):
    ...
    def __post_init__(self):
        ...
        self.add_node("fft", "FFT", shape="rectangle", color="k", penwidth=2.5)
        self.add_node(
            "filter", "Filter", shape="rectangle", color="darkred", penwidth=2.5
        )
        self.add_node("ifft", "inverse FFT", shape="rectangle", color="k", penwidth=2.5)
        self.add_node(
            "response", "response", shape="rectangle", color="darkgreen", penwidth=2
        )
...
class ErrorField(DiagramBase):
    ...
    def build_links(self):
        ...
        for i in range(1, 4):
            self.add_edge("error", f"pdf{i}", label=f" Plasma{i}", **self.highlight)
...
class Diagram(DiagramBase):
    ...
    nodesep: float = 0.25
    ranksep: float = 0.35
    theta: list[float] = field(default_factory=lambda: [1.5, 1.5, 1.5, 1.5, 2, 2, 5])
    pdf: list[str] = field(
