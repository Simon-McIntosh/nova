...
class Base(ModelBase):
    ...
    def build(self):
        ...
        self.data = xarray.Dataset(
            attrs=dict(ncoil=self.ncoil, nyquist=self.ncoil // 2)
        )
    ...
    def _filter(self, label: str):
        ...
        return self.data.filter[:, 0].sel(signal=label) + 1j * self.data.filter[
            :, 1
        ].sel(signal=label)
    ...
    def _predict(self, delta, signal: str, ndiv: int):
        ...
        return (
            np.fft.irfft(np.fft.rfft(delta) * self.filter[signal], n=ndiv)
            * ndiv
            / self.ncoil
        )
    ...
    def peaktopeak(self, fieldline=None, modes=3, axis_offset=False):
        ...
        if axis_offset:
            fieldline_hat[..., 1] = 0
        fieldline = np.fft.irfft(fieldline_hat[..., : modes + 1], fieldline.shape[-1])
        return fieldline.max(axis=-1) - fieldline.min(axis=-1)

    @property
    def axis_offset(self):
        """Return prediction for magnetic axis offset."""
        return np.fft.rfft(self.fieldline.data)[..., 1] / (self.fieldline.shape[1] // 2)
    ...
    def plot_deviation(self, axes, phi, benchmark, legend=True, sample=0):
        ...
        axes.plot(
            phi, model, "C3-.", label=f"inference H={self.peaktopeak(model):1.2f}"
        )
        model_delta = model.max() - model.min()
        axes.set_ylim(
            model.min() - 0.05 * model_delta, model.max() + 0.05 * model_delta
        )
    ...
    def plot_benchmark(self, simulation: str, title=True):
        ...
        for i, label in enumerate([r"$\Delta r$", r"$r\Delta \phi$"]):
            axes[i].bar(dataset.index, dataset.delta[:, i], width=0.8, color="C1")
            axes[i].set_xticks([])
    ...
    def build(self, phase_shift=False):
        ...
        for mode in self.data.mode.values[1:]:
            ...
            result = scipy.optimize.minimize(
                self.fit,
                self.sead(r_hat[mode], t_hat[mode], phase_shift),
