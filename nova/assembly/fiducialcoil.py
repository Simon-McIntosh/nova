...
class FiducialCoil(Plotter):
    ...
    def load_case(self, decimate=0.9):
        ...
        case = case.clip((-np.sin(np.pi / 18), np.cos(np.pi / 18), 0), origin=(0, 0, 0))
        case = case.clip(
            (np.sin(np.pi / 18), np.cos(np.pi / 18), 0), origin=(0, 0, 0), invert=False
        )
        if decimate == 0:
            return case
...
if __name__ == "__main__":
    fiducialcoil = FiducialCoil("fiducial", 10, file="k0_wp")
    # fiducialcoil.mesh = fiducialcoil.mesh.slice((0, 0, 1))
    fiducialcoil.warp(150, opacity=0, displace="delta")
