...
class FiducialError:
    ...
    def plot(self, index, factor=500):
        ...
        for index, coord in zip(((0, 0), (0, 1), (1, 0)), ((0, 2), (1, 2), (0, 1))):
            ...
            axes[index].plot(
                centerline[:, coord[0]] + factor * centerline_delta[:, coord[0]],
                centerline[:, coord[1]] + factor * centerline_delta[:, coord[1]],
                "-",
                label="CCL",
            )
            axes[index].plot(
                centerline[:, coord[0]] + factor * fit_delta[:, coord[0]],
                centerline[:, coord[1]] + factor * fit_delta[:, coord[1]],
                "-",
                label="fit",
            )
    ...
    def plot_coilset(self, factor=500):
        ...
        for axid in range(2):
            axes[axid].plot(centerline[:, 0], centerline[:, 2], "--", color="gray")
        for index in range(18):
            ...
            axes[axid].plot(
                centerline[:, 0] + factor * fit_delta[:, 0],
    ...
    def plot_error(self):
        ...
        for i, (coord, radius, title) in enumerate(
            zip(["ry", "rx", "rz"], [4.5, 4.5, 0.25], ["pitch", "roll", "yaw"])
        ):
            delta = (
                1e3
                * radius
                * self.data.fit_translate.sel(translate=coord)[loc]
                * np.pi
                / 180
            )
