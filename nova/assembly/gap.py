...
class GapData:
    ...
    def plot_waveform(self, signal: str, simulation: str, color: str):
        ...
        phi = np.linspace(
            0, 2 * np.pi - np.pi / self.data.nyquist, 20 * self.data.nyquist
        )
        waveform = 0
        amplitude = self.data.fft.sel(
            simulation=simulation, signal=signal, coefficient="amplitude"
        ).data
        phase = self.data.fft.sel(
            simulation=simulation, signal=signal, coefficient="phase"
        ).data
...
class UniformGap(ModelData):
    ...
    def read_gapfile(self):
        ...
        if self.gapfile in ["Gap_Size_18_Coils", "F4E_vgap"]:
            ...
            columns = {column: column.replace("_", "").lower() for column in gapdata}
...
class WedgeGap(ModelData):
    ...
    def build(self):
        ...
        self.data = self.data.merge(
            GapData(
                ...
                self.data.rotate[..., 2].data * 1e3 * self.length["yaw"],
            ).data
        )
        return self.store()
    ...
    def build_dataset(self):
        ...
        self.data["point_gap"] = xarray.DataArray(0.0, self.data.coords)
