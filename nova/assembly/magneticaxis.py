...
plt.plot(radius * np.cos(phi), radius * np.sin(phi), "-.", color="gray", label="wall")

delta_r = radius + deviation
plt.plot(
    delta_r * np.cos(deviation.phi),
    delta_r * np.sin(deviation.phi),
    "-",
    color="C2",
    label=r"$h(\phi)$",
)
...
plt.plot(
    n1_delta_r * np.cos(deviation.phi),
    n1_delta_r * np.sin(deviation.phi),
    "--",
    color="C2",
    label=rf"$n_{{0to{modes}}}$ fit",
)
...
plt.plot(*zeta_r, "C4X", ms=6, label=rf"$\zeta_{{B_r}} ${np.linalg.norm(zeta_r):1.2f}")
plt.plot(
    *zeta_t, "C5X", ms=6, label=rf"$\zeta_{{B_\phi}} ${np.linalg.norm(zeta_t):1.2f}"
)
