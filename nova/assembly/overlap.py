...
class Model(Dataset):
    ...
    points: ClassVar[dict] = dict(
        A=np.array([2612.7, -3688.8]),
        C=np.array([0, 6277.3]),
        E=np.array([-4240.3, -3683.8]),
        F=np.array([0, -6303.5]),
    )
    coordinates: ClassVar[list[str]] = [
        "radial",
        "tangential",
        "vertical",
        "pitch",
    ...
    overlap_limit: ClassVar[dict[int, float]] = {1: 4.2e-4, 2: 2.5e-4, 3: 0.8e-4}
    ...
    def load_filter(self):
        ...
        self.filter = {plasma: self._filter(plasma).data for plasma in range(1, 4)}

    def build(self):
        """Load data extracted from Y26X3K_v1_0."""
        data = netCDF("displace", directory="root", path=self.path).load().data
        self.data = xarray.Dataset(data.coords)
        self.data["coefficient"] = ["real", "imag"]
        self.data["signal_fft"] = xarray.DataArray(0.0, self.data.coords)
        self.data["response_fft"] = xarray.DataArray(0.0, self.data.coords)
        self.data["filter"] = xarray.DataArray(0.0, self.data.coords)
        ...
        _filter = (
            self.data.response_fft[..., 0] + 1j * self.data.response_fft[..., 1]
    ...
    def _filter(self, plasma: int):
        ...
        return self.data.filter[..., 0].sel(plasma=plasma) + 1j * self.data.filter[
            ..., 1
        ].sel(plasma=plasma)
    ...
    def predict(self, plasma: int, *signals):
        """Return prediction."""
        response = 0
        for signal, coordinate in zip(
            signals, ["radial", "tangential", "vertical", "pitch", "roll", "yaw"]
        ):
            if signal is None:
                continue
