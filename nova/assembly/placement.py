...
class Placement:
    ...
    def make_frame(self, t):
        ...
        if last_sample >= 1 / self.samples_per_second:
            self.sample_time = t
            self.sample = self.rng.uniform(-self.bound, self.bound,
                                           size=self.ncoil)
        ...
        plt.gca().clear()
        plt.bar(self.index, self.sample, color='C0', label='gap',
                zorder=-2, width=0.9)
        plt.bar(self.index,
                self.bound * np.cos((self.index-1) * 2*np.pi / self.ncoil),
                color='gray', zorder=-3, width=0.9)

        plt.plot(1 + self.phi * self.ncoil / (2*np.pi),
                 self.amplitude * np.cos(self.phi + self.phase),
                 'C3-.', zorder=1)

        plt.plot(self.index, self.bound * np.ones(self.ncoil),
                 '--', color='gray')
        plt.plot(self.index, -self.bound * np.ones(self.ncoil),
                 '--', color='gray')
        axes = plt.gca()
