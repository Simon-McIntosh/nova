...
class SpacialAnalyzer:
    ...
    columns: ClassVar[list[str]] = ["collection", "group", "point", "x", "y", "z"]
    ...
    def __post_init__(self):
        ...
        self.files = (
            dict(
                nominal="nominal",
                nominal_ccl="nominal_ccl",
                reference_ccl="reference_ccl",
            )
            | self.files
        )
        self.extract_datum()
    ...
    def datum_error(self, datum, nominal_ccl):
        ...
        return np.mean(
            self.rotate.clock(nominal[0])[..., 1] ** 2
    ...
    def extract_datum(self):
        ...
        opt = minimize(self.datum_error, (0, 0, 0), args=(nominal_ccl,), method="SLSQP")
        if not opt.success:
    ...
    def read_ccl(self, filename: str):
        ...
        dataframe[["coil", "point"]] = dataframe.point.str.split("-", expand=True)
        ...
        dataarray = xarray.DataArray(
            ...
            coords=dict(coil=coils, fiducial=fiducial, cartesian=list("xyz")),
        )
        for i, coil in enumerate(coils):
            dataarray[i] = dataframe.loc[dataframe.coil == coil, ["x", "y", "z"]].values
        dataarray.coords["coil"] = [
            int("".join(filter(str.isdigit, coil))) for coil in dataarray.coil.values
        ]
        dataarray.attrs |= dataframe.attrs
        dataarray = dataarray.sel(fiducial=list("ABCDEFGH"))
        dataarray["clock"] = np.mean(
            np.arctan2(dataarray[..., 1], dataarray[..., 0]), axis=1
        )
    ...
    def read_points(self, filename: str) -> xarray.DataArray:
        ...
        dataarray = xarray.DataArray(
            ...
            coords=dict(fiducial_ex=dataframe.index.values, cartesian=list("xyz")),
        )
        dataarray.attrs |= dataframe.attrs
        return dataarray
    ...
    def to_file(self, file, data: xarray.DataArray, collection: str, group: str = ""):
    ...
    def write_ccl(self, file, dataarray: xarray.DataArray, collection: str, group: str):
        """Write ccl dataarray to file."""
        for coil in dataarray.coil.values:
            dataframe = self.to_dataframe(dataarray.sel(coil=coil), collection, group)
            dataframe["point"] = [
    ...
    def write_dataarray(
        self, file, dataarray: xarray.DataArray, collection: str, group: str
    ):
