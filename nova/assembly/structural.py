...
class Points(ModelData):
    ...
    def attrs(self):
        ...
        return {
            ...
            for attr in ["filename", "folder", "origin", "ncoil", "cluster"]
        }
    ...
    def initialize_dataset(self):
        ...
        self.data["points"] = xarray.DataArray(0.0, self.data.coords)
    ...
    def plot(self, coefficient="amplitude", scenario="TFonly"):
        ...
        for i, response in enumerate(["radial", "tangential"]):
            axes[i].bar(
                self.data.mode,
                self.data.fft.sel(
                    scenario=scenario, response=response, coefficient=coefficient
                ),
            )
...
class BaseTransform:
    ...
    def build(self):
        ...
        self.data.filter[..., 2] = (
            self.data.response_fft[..., 2] / self.data.signal_fft[..., 2]
        )
        self.data.filter[..., 3] = (
            self.data.response_fft[..., 3] - self.data.signal_fft[..., 3]
        )
    ...
    def response_coef(self):
        ...
        return self.data.response_fft[..., 0] + 1j * self.data.response_fft[..., 1]
...
class Transform:
    ...
    def __post_init__(self):
        ...
        self.data["filter"] = BaseTransform(
            self.data.signal_fft, self.data.response_fft
        ).data.filter
...
class Model(ModelBase):
    ...
    def _filter(self, label: str):
        ...
        return self.data.filter[..., 0].sel(response=label) + 1j * self.data.filter[
            ..., 1
        ].sel(response=label)
    ...
    def build_gap_filter(self):
        ...
        for mode in self.data.mode.values[1:]:
            self.data.filter[mode, :, 0] = Transform(f"k{mode}", "gap").data.filter[
                mode
            ]
        self.load_filter()
    ...
    def build_yaw_filter(self, simulation="w3"):
        ...
        self.data.filter[..., 2, :] = BaseTransform(
            transform.signal_fft.sel(signal="yaw"), data.fft
        ).data.filter
        self.load_filter()
    ...
    def build_roll_filter(self, simulation="w4"):
        ...
        data = transform.response_delta.to_dataset()
        data = data.rename(dict(response_delta="delta"))
        data.delta[:, 0] -= self.predict_radial(transform.gap, yaw=transform.yaw)
        data.delta[:, 1] -= self.predict_tangential(transform.gap, yaw=transform.yaw)
        self.fft(data)
        self.data.filter[..., 1, :] = BaseTransform(
            transform.signal_fft.sel(signal="roll"), data.fft
        ).data.filter
        self.load_filter()
    ...
    def predict(self, response, *signals):
        ...
        for i, signal in enumerate(signals):  # gap, roll, yaw
            if signal is None:
                continue
            waveform += np.fft.irfft(np.fft.rfft(signal) * self.filter[response][:, i])
        return waveform

    def plot_benchmark(self, simulation: str, title=True):
        """Plot structural model results."""
        transform = Transform(simulation)
        transform.data.response_delta[:] -= transform.data.response_delta.mean(axis=0)
        ...
        axes[1].bar(
            ...
            transform.data.response_delta[:, 0],
            width=0.85,
            color="C1",
            label="ANSYS",
        )
        ...
        axes[2].bar(
            transform.data.index,
            transform.data.response_delta[:, 1],
            width=0.85,
            color="C1",
            label="ANSYS",
        )
        ...
        axes[1].bar(
            transform.data.index, model_radial, width=0.45, color="C2", label="Proxy"
        )
        model_tangential = self.predict_tangential(*transform.data.signal_delta.data.T)
        axes[2].bar(
            transform.data.index,
            model_tangential,
            width=0.45,
