...
class PlasmaGap(Plot, Proximate, Operate):
    ...
    def solve(self, points, angle, label=None):
        ...
        probes = (
            np.einsum("ik,j->ijk", np.c_[np.cos(angle), np.sin(angle)], self.nodes)
            + points
        )
        target = Target()
    ...
    def load_operators(self):
        ...
        if self.number is not None:
            self.node_number = self.data.sizes["nodes"]
            self.gap_number = self.data.sizes["name"]
    ...
    def shape(self):
        ...
        return self.data.sizes["name"], self.data.sizes["nodes"]
    ...
    def bisect(self, gap):
        """Return gap indicies."""
        return select.bisect_2d(self.nodes, gap) + self.node_number * np.arange(
            self.data.sizes["name"]
        )
