"""Measure sensor noise, drift and repeat scatter from waveform arrays.
...
class WaveformArrays(Protocol):
    """Array bundle consumed by the noise kernels.

    Machine adapters may use any concrete record with these attributes.  The
    kernels depend only on numerical arrays and channel-keyed mappings.
    """
...
class NoiseFit:
    """Scatter and linear drift recovered from one waveform."""
...
def measure_noise(time: np.ndarray, values: np.ndarray) -> NoiseFit:
    ...
    if count < MINIMUM_NOISE_SAMPLES:
        return NoiseFit(float("nan"), float("nan"), count)
    clock = time[finite]
    signal = values[finite]
    span = float(clock.max() - clock.min())
    if not math.isfinite(span) or span <= 0.0:
        return NoiseFit(float(np.std(signal)), 0.0, count)
    slope, intercept = np.polyfit(clock, signal, 1)
    residual = signal - (slope * clock + intercept)
    return NoiseFit(float(np.std(residual)), float(slope), count)
...
def measure_noise_envelope(
    waveforms: Iterable[WaveformArrays],
    *,
    ...
    for waveform in waveforms:
        ...
        for name, signal in sorted(waveform.probes.items()):
            if channels is not None and name not in channels:
                continue
            fit = measure_noise(waveform.time, signal)
            scatter, drift = fit.scatter, fit.drift_rate
...
def measure_repeat_scatter(
    family: str,
    shots: Sequence[int],
    waveforms: Mapping[int, WaveformArrays],
    *,
