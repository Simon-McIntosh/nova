...
class Circuit(Plot, netCDF, FrameSetLoc):
    ...
    def edge_list(self, circuit: str):
        ...
        return {
            edge: (np.where(col == -1)[0][0], np.where(col == 1)[0][0])
            for edge, col in zip(self.data.edge.values, incidence_matrix.values.T)
    ...
    def _extract_linked_coils(self, loop):
        ...
        index = np.array(
            [
                np.where(coil == np.array(loop["edge"]))[0][0]
                for coil in self.data.coil.data
                if coil in np.array(loop["edge"])
            ],
            dtype=int,
        )
        if len(index) < 2:
            return None
        return {attr: list(np.array(loop[attr])[index]) for attr in ["edge", "sign"]}
    ...
    def links(self):
        ...
        return [
            loop
            for circuit in self.data.circuit.data
            if len(loops := self.edge_loops(circuit)) == 1
            and (loop := self._extract_linked_coils(loops[0])) is not None
        ]
        ...
        for link in self.links:
            ...
            for i in range(len(link["edge"]) - 1):
                matrix[index] = np.sum(
                    [
                        reference,
                        np.where(coils == link["edge"][i + 1], -link["sign"][i + 1], 0),
                    ],
                    axis=0,
                )
                index += 1
    ...
    def link(self):
        ...
        for circuit in self.data.circuit.data:
            ...
            sort = sorted(
                zip(index, factor), key=lambda x: self.frame.index.get_loc(x[0])
            )
