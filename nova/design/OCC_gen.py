...
class OCC(object):
    def __init__(self, config, setup, family="S", nTF=16, obj="L"):
        ...
        self.cage = coil_cage(nTF=nTF,rc=self.tf.rc,ny=3,
                              plasma={'setup':setup},
                              coil=self.tf.x['cl'])
        self.eq = EQ(self.sf,self.pf,dCoil=1.5,sigma=0,
                     boundary=self.sf.get_sep(expand=0.4),n=1e3)
        self.eq.plasma()
    ...
    def support_arm(L, coil, TFloop):
        dl = np.sqrt(
            (coil["r"] - TFloop["r"](L)) ** 2 + (coil["z"] - TFloop["z"](L)) ** 2
        )
        return dl
    ...
    def connect(self, coil, loop, edge=0.15, hover=0.1, argmin=60):
        L = minimize_scalar(
            OCC.support_arm, method="bounded", args=(coil, loop), bounds=[0, 1]
        ).x
    ...
    def draw_OIS(self, L, width, thickness, TFloop):
        ...
        L = minimize_scalar(
            OCC.OIS_placment, method="bounded", args=(TFloop, (ro, zo)), bounds=[0, 1]
        ).x
    ...
    def plot(self):
        ...
        geom.polyfill(nodes[:, 0], nodes[:, 1], color=0.4 * np.ones(3))
        plt.plot(
            self.Gsupport["radius"] * np.ones(2),
    ...
    def ansys(self, plot=False, nl=250, nr=5, ny=5):
        ...
        ans.f.write("! body applied to nodes of winding-pack")
        ans.f.write(" in cartisean system x,y,z\n")
        ans.f.write("! winding-pack must be labled")
        ans.f.write(" as named-selection 'wp'\n")
        ans.f.write(
            "\nlocal,11,1,{:1.9f},0,{:1.9f},0,90,0".format(self.sf.mo[0], self.sf.mo[1])
        )
        ...
        *enddo
        ...
        # construct TF interpolators
        self.tf.loop_interpolators(offset=0, full=True)
        TFloop = self.tf.fun["cl"]
        ...
        for i in range(ngrid["nr"]):
            for j in range(ngrid["nt"]):
                ...
                l_map[i, j] = L
                dr_map[i, j] = np.sqrt((x - xl) ** 2 + (z - zl) ** 2) * np.sign(
                    np.dot([x - xl, z - zl], [x - self.sf.mo[0], z - self.sf.mo[1]])
                )
        ...
        for i, l in enumerate(l_data):
            ...
            for j, dr in enumerate(dr_data):
                for k, dy in enumerate(dy_data):
                    ...
                    Fb = self.ff.topple(
                        point, J, self.cage, self.eq.Bpoint, method="BS"
                    )  # body force
                    ...
                    if plot:
                        ...
                        plt.plot(point[0], point[2], "o", color=0.15 * np.ones(3), ms=3)
        print(
            "\n",
        ...
        for var in ["x", "y", "z"]:
            ...
            ans.load("Fbody_{}".format(var), Fbody[var], [l_data, dr_data, dy_data])
