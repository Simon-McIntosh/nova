...
class divertor(object):
    ...
    def place(self, **kwargs):
        ...
        for leg in list(self.sf.legs)[2:]:
            ...
            for flip, direction, theta_end, psi_target in zip(
                Flip, Direction, Theta_end, Phi_target
            ):
                X, Z = self.match_psi(
                    Xo,
                    Zo,
        ...
        if self.sf.nleg == 6:  # SF
            ...
            x, z = self.connect(
                self.sf.Xpsi - self.div_ex * dpsi, ["inner", "outer"], [-1, 0]
            )
            Xb, Zb = self.append(Xb, Zb, x, z)
            self.segment["divertor"] = {"x": Xb, "z": Zb}  # store diveror
    ...
    def join(self, main_chamber):
        ...
        istart = np.argmin((x - self.sf.Xpoint[0]) ** 2 + (z - self.sf.Xpoint[1]) ** 2)
        ...
        # divertor outer wall
        xd, zd, i_out = self.intersect(
            x, z, xd, zd, s=0.075, offset=self.setup.firstwall["dx_div"]
        )
        self.segment["divertor_outer"] = {"x": xd, "z": zd}

        xloop = np.append(
            self.segment["divertor_inner"]["x"], x[i_in["istart"] : i_out["istart"]]
        )
        zloop = np.append(
            self.segment["divertor_inner"]["z"], z[i_in["istart"] : i_out["istart"]]
        )
        ...
        # gap to blanket
        xd, zd, i_gap = self.intersect(
            x, z, xd, zd, offset=self.setup.firstwall["bb_gap"]
        )
    ...
    def match_psi(
        ...
        flag = 0
        for i in range(Nmax):
            X, Z, phi = self.blend_target(
                Xo, Zo, dPlate, L, direction, theta_end, theta_sign, graze, x2m, Nplate
            )
    ...
    def blend_target(
        ...
        if abs(target_angle - trans_angle) > 0.01 * np.pi:
            ...
            accute = False
        X, Z = self.extend_target(
            X, Z, dL, N, x2s, theta_end, theta_sign, direction, graze, accute
        )  # transition graze
...
class main_chamber(object):
    ...
    def generate(
        ...
        self.set_bounds()
        self.config = {"dx": dx, "psi_n": psi_n, "flux_fit": flux_fit, "Nsub": 100}
        self.config["eqdsk"] = []
    ...
    def add_bound(self, sf):
        ...
        self.shp.add_bound(
            {"x": Xpoint[0] + 0.12 * sf.shape["a"], "z": Xpoint[1]}, "internal"
        )
        self.shp.add_bound(
            {"x": Xpoint[0], "z": Xpoint[1] - 0.01 * sf.shape["a"]}, "internal"
        )

        if self.config["flux_fit"]:  # add flux boundary points
            sf.get_LFP()  # get low feild point
            rflux, zflux = sf.first_wall_psi(psi_n=self.config["psi_n"], trim=False)[:2]
            ...
            if np.max(dL) > 3 * np.median(dL) or np.argmax(zflux) == len(zflux) - 1:
                ...
                rflux, zflux = geom.rzSLine(rflux, zflux, int(self.config["Nsub"] / 2))
