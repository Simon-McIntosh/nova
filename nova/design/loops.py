...
def unwind(p, angle=2.5, plot=False):
    ...
    string = String(np.array([p["x"], p["z"]]).T, angle=angle, verbose=False)
    if plot:
        string.plot()
...
def set_oppvar(xo, oppvar):  # set optimization bounds and normalize
    ...
    for i, var in enumerate(oppvar):
        ...
        xnorm[i] = (xo[var]["value"] - xo[var]["lb"]) / (xo[var]["ub"] - xo[var]["lb"])
        bnorm[i, :] = [0, 1]
    return xnorm, bnorm
...
class Sloop(object):  # polybezier
        ...
        for key in inputs:
            ...
            for keyo_ in keyo:
                if keyo_ in self.xo:
                    ...
                    self.xo[keyo_] = set_limit(self.xo[keyo_], limits=self.limits)
        self.set_symetric()
        self.set_tension()

    def verticies(self):
        x1, x2, z2, height, top, bottom, upper, lower, dz, ds, alpha_s = self.get_xo()
    ...
    def plot(self, inputs={}, ms=8):
        ...
        for po in self.po:
            ...
            plt.plot(po["p1"]["x"], po["p1"]["z"], "o", color=c1, ms=2 * ms, zorder=6)
            plt.plot(po["p1"]["x"], po["p1"]["z"], "o", color=c2, ms=ms, zorder=7)
            plt.plot(
                [po["p3"]["x"], po["p2"]["x"]],
                [po["p3"]["z"], po["p2"]["z"]],
                color=c1,
            ...
            plt.plot(po["p2"]["x"], po["p2"]["z"], "o", color=c1, ms=2 * ms, zorder=6)
            plt.plot(po["p2"]["x"], po["p2"]["z"], "o", color=c2, ms=ms, zorder=7)
        plt.axis("equal")
        plt.axis("off")
    ...
    def avalible_data(self, verbose=True):
        if len(self.loop_dict) == 0:
            ...
            for family in self.data_frame:
                datatxt += "\n\nloop type {}:\n{}".format(
                    family, self.data_frame[family].fillna("")
                )
        if verbose:
            print(datatxt)
        else:
...
if __name__ is "__main__":  # plot loop classes
    ...
    # loop.draw()
    profile = Profile(
        "demo", family="D", load=True, part="TF", nTF=16, obj="L", npoints=500
    )
