...
class INV(object):
    def __init__(
        ...
        if tf is not None:
            dx = [self.coilset["coil"][name]["dx"] for name in self.coilset["coil"]]
    ...
    def fix_boundary(self, psi=True, field=False, Xpoint=False, npoint=25):
    ...
    def get_flux(
        self, centre=0, width=363 / (2 * np.pi), array=[-0.5, 0, 0.5], **kwargs
    ):
    ...
    def set_swing(self, **kwargs):
        ...
        self.swing = {
            "flux": flux,
            "It": np.zeros((self.nS, self.nC)),
            "rss": np.zeros(self.nS),
            "FxPF": np.zeros(self.nS),
            "FzPF": np.zeros(self.nS),
            "FsepCS": np.zeros(self.nS),
            "FzCS": np.zeros(self.nS),
            "Isum": np.zeros(self.nS),
            "IsumCS": np.zeros(self.nS),
            "IsumPF": np.zeros(self.nS),
            "Imax": np.zeros(self.nS),
            "IcCSmax": np.zeros(self.nS),
            "Tpol": np.zeros(self.nS),
            "rss_bndry": np.zeros(self.nS),
            "z_offset": np.zeros(self.nS),
            "dTpol": 0,
    ...
    def inductance(self):
        ...
        turns = np.array(
            [self.coilset["coil"][name]["Nt"] for name in self.coil["active"]]
        )
        turns = np.dot(turns.reshape(-1, 1), turns.reshape(1, -1))
        fillaments = np.array(
            [self.coilset["coil"][name]["Nf"] for name in self.coil["active"]]
        )
        fillaments = np.dot(fillaments.reshape(-1, 1), fillaments.reshape(1, -1))
    ...
    def append_subcoil(self, subcoil):
        for subname in subcoil.keys():
            ...
            for key, var in zip(
                ["x", "z", "If", "dx", "dz", "subname"],
                [coil["x"], coil["z"], coil["If"], coil["dx"], coil["dz"], subname],
            ):
                self.coil[state][name][key] = np.append(
                    self.coil[state][name][key], var
                )
            self.coil[state][name]["It"] = np.sum(self.coil[state][name]["If"])
    ...
    def get_point(self, **kwargs):
        ...
        if "norm" in keys and "point" not in keys:
            delta = kwargs["norm"] * np.array([dx, dz]) / np.sqrt(dx**2 + dz**2)
            x += delta[1]
            z -= delta[0]
        return x, z
        ...
        for state, marker in zip(["passive", "active"], ["o", "*"]):
            for name in self.coil[state].keys():
                if name != "Plasma" or plasma:
                    ...
                    for x, z in zip(
                        self.coil[state][name]["x"], self.coil[state][name]["z"]
                    ):
    ...
    def add_fix(self, x, z, value, Bdir, BC, factor):
        var = {"x": x, "z": z, "value": value, "Bdir": Bdir, "BC": BC, "factor": factor}
        nvar = len(x)
        self.fix["n"] += nvar
    ...
    def get_boundary(self, npoint=21, alpha=0.995):
        try:
            ...
            Xpoint = True
        except TypeError:  # X-point not present
            x = np.array(
                [self.sf.x[0], self.sf.x[-1], self.sf.x[-1], self.sf.x[0], self.sf.x[0]]
            )
            z = np.array(
                [self.sf.z[0], self.sf.z[0], self.sf.z[-1], self.sf.z[-1], self.sf.z[0]]
            )
            L = geom.length(x, z, norm=False)[-1]
            dndL = npoint / L
            xloop, zloop = [], []
            for i in range(4):
                xs, zs = x[i : i + 2], z[i : i + 2]
                Ls = geom.length(xs, zs, norm=False)[-1]
                ns = int(Ls * dndL)
                print(xs, zs)
                xs, zs = geom.xzInterp(xs, zs, npoints=ns)
                print(xs[0], xs[-1], zs[0], zs[-1])
                xloop.extend(xs[:-1])
                zloop.extend(zs[:-1])
            x, z = xloop, zloop
            Xpoint = False
    ...
    def fix_null(self, factor=1, **kwargs):
        ...
        self.add_fix([x], [z], [0.0], np.array([[1.0], [0.0]]).T, ["Bx"], [factor])
        self.add_fix([x], [z], [0.0], np.array([[0.0], [1.0]]).T, ["Bz"], [factor])
    ...
    def add_B(self, B, Bdir, factor=1, zero_norm=False, **kwargs):
        ...
        self.add_fix(
            [x], [z], [B], np.array([[Bdir[0]], [Bdir[1]]]).T, ["Bdir"], [factor]
        )
        if zero_norm:
            self.add_fix(
                [x], [z], [0], np.array([[-Bdir[1]], [Bdir[0]]]).T, ["Bdir"], [factor]
            )
    ...
    def add_theta(self, theta, factor=1, graze=1.5, **kwargs):
        ...
        Bp = Bphi / np.sqrt(
            (np.sin(theta * np.pi / 180) / np.sin(graze * np.pi / 180)) ** 2
        )
    ...
    def plot_fix(self, tails=True):
        ...
        if self.fix["n"] > 0:
            ...
            for x, z, bc, bdir, w in zip(
                self.fix["x"], self.fix["z"], self.fix["BC"], self.fix["Bdir"], weight
            ):
                ...
                if "psi" in bc:
                    ...
                    plt.plot(x, z, marker, color=[1, 1, 1], markersize=0.3 * size)
                else:
                    if bc == "Bdir":
                        norm = Bdir_norm
                    ...
                    plt.plot(
                        x, z, marker, color=color, markersize=size, markeredgewidth=mew
                    )
                if tails:
                    plt.plot(
                        [x, x + direction[0] * norm * w],
    ...
    def add_value_coil(self, bc, xf, zf, x, z, bdir, dx, dz):
        if "psi" in bc:
            # flux Wb/rad.A
            value = self.Iscale * cc.mu_o * cc.green(xf, zf, x, z, dXc=dx, dZc=dz)
        else:
            # field T/A
    ...
    def get_weight(self):
        ...
        if n > 0:
            for i, (xf, zf, bc, bdir, factor) in enumerate(
                zip(Xf, Zf, BC, Bdir, self.fix["factor"])
            ):
    ...
    def grid_CS(self, n, Xo=2.9, Zbound=[-10, 9], dx=0.818, gap=0.1, fdr=1):
        ...
        Le = np.linspace(
            Zbound[0] - gap / 2, Zbound[-1] + gap / 2, nCS + 1
        )  # coil edges
        self.update_coils()
    ...
    def move_PF(self, name, AR=0, **kwargs):
        ...
        if (
            "point" in kwargs.keys()
            or "Lout" in kwargs.keys()
        ...
        if AR > 0:
            ...
            AR = 1 / (1 - AR)
        dA = self.coilset["coil"][name]["dx"] * self.coilset["coil"][name]["dz"]
    ...
    def store_update(self, extent="full"):
        if extent == "full":
            ...
            self.log["plasma"].append(self.copy_coil(self.eq.plasma).copy())
    ...
    def poloidal_angle(self):  # calculate poloidal field line angle
        if "Bdir" in self.fix["BC"]:
            ...
            B = self.sf.Bpoint([self.fix["x"][index][-1], self.fix["z"][index][-1]])
    ...
    def set_Io(self):  # set lower/upper bounds on coil currents (Jmax)
        self.Io = {
            "name": self.adjust_coils,
            "value": np.zeros(self.nC),
            "lb": np.zeros(self.nC),
            "ub": np.zeros(self.nC),
        }
    ...
    def solve_slsqp(self, flux):  # solve for constrained current vector
        ...
        tol = 1e-2 * np.ones(
            2 * self.nPF + 2 + 2 * (self.nCS - 1) + 2 * (self.nCS + 1)
        )  # 1e-3
    ...
    def update_area(self, relax=1, margin=0):
        It = self.swing["It"][np.argmax(abs(self.swing["It"]), axis=0), range(self.nC)]
        for name in self.PFcoils:
            if name in self.adjust_coils:
    ...
    def update_position_vector(self, Lnorm, grad):
        ...
        rss_str = "\r{:d}) rss {:1.2f}mm ".format(self.iter["position"], 1e3 * rss)
    ...
    def tock(self):
        ...
        print(
            "total {:1.0f}s speedup {:1.0f}%".format(
                self.ttotal, 100 * self.fcpu / self.ncpu
            )
        )
        print("")
    ...
    def update_current(self):
        ...
        for j, name in enumerate(self.coil["active"]):
            ...
            for i in range(Nfilament):
                subname = name + "_{:1.0f}".format(i)
                self.coilset["subcoil"][subname]["If"] = self.If[j] * self.Iscale
...
class SWING(object):
    def __init__(
        self, inv, sf, rss_limit=0.10, wref=0, nswing=2, plot=False, output=True
    ):
        self.nswing = nswing
        self.inv = inv
    ...
    def get_rss(self, centre):
        self.inv.set_swing(
            centre=centre, width=self.wref, array=np.linspace(-0.5, 0.5, 2)
        )
        rss = self.inv.update_position(self.Lnorm, update_area=True)
        return float(self.rss_limit - rss)

    def find_root(self, slim):
        swing = brentq(self.get_rss, slim[0], slim[1], xtol=0.1, maxiter=500, disp=True)
        return swing
