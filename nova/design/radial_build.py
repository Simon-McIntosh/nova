...
class RB(object):
    ...
    def generate(
        ...
        if mode == "eqdsk":  # read first wall from eqdsk
            ...
            if DN:  # double null
                ...
                x, z = self.upper_lower(xu, zu, xl, zl, Zjoin=self.sf.Mpoint[1])
                self.segment["first_wall"] = {"x": x, "z": z}
                xfw, zfw = self.upper_lower(
                    blanket_u[0]["x"][::-1],
                    blanket_u[0]["z"][::-1],
                    blanket_l[0]["x"],
                    blanket_l[0]["z"],
    ...
    def plot(self):
        ...
        geom.polyfill(
            self.segment["divertor"]["x"],
            self.segment["divertor"]["z"],
    ...
    def place_blanket(self, select="upper", store=True, plot=False):
        blanket = wrap(self.segment["vessel_gap"], self.segment["inner_loop"])
        blanket.sort_z("inner", select=select)
        blanket.offset(
            "outer", self.setup.build["BB"], ref_o=3 / 10 * np.pi, dref=np.pi / 3
        )
    ...
    def vessel_fill(self, gap=True):
        ...
        shp.add_bound(
            {"x": xd, "z": zd - self.setup.firstwall["vv_gap"]}, "internal"
        )  # for gap below divertor
        # shp.plot_bounds()
        shp.minimise()
        # shp.loop.plot()
        x = profile.loop.draw()
        xin, zin = x["x"], x["z"]
        loop = Loop(xin, zin)
        x, z = loop.fill(dt=self.setup.build["VV"], ref_o=2 / 8 * np.pi, dref=np.pi / 6)
        shp.clear_bound()
...
class wrap(object):
    ...
    def fill(self, plot=False, color="C0"):  # minimization focused search
        ...
        if not concentric:
            ...
            for i, dl in enumerate([[0, 0.5], [0.5, 1]]):
                lo = self.sead(dl)
                L = minimize(
                    self.cross, lo, method="L-BFGS-B", bounds=([0, 1], [0, 1])
                ).x
    ...
    def plot(self, color="C1", plot_loops=False):
        ...
        if plot_loops:
            plt.plot(self.segment["x"], self.segment["z"], color=0.75 * np.ones(3))
            for loop in self.loops:
                x, z = self.get_segment(loop)
                plt.plot(x, z, "-")
        ...
        if plot:
            ...
            for i in np.arange(Lindex[0], Lindex[1] - 1):
                ...
                if flag is 0 and label is not None:
                    ...
                    plt.fill(
                        Xfill, Zfill, facecolor=color, alpha=alpha, edgecolor="none"
                    )
