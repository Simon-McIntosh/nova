...
class coil_class(dict):
    ...
    def add_coilset(self, coilset, label=None):
        ...
        if label:
            self['index'][label] = {'name': list(coilset['coil'].keys()),
    ...
    def mesh_coil(coil, dCoil=None, name=None):
        ...
        if name is not None:
            ...
            index = range(Nf)
        subcoil = pd.DataFrame(index=index, columns=['x', 'z', 'dx', 'dz',
                                                     'If', 'rc',
                                                     'cross_section'])
    ...
    def index_coilset(self):  # append coil-subcoil map in index
        ...
        for i, name in enumerate(self['coil']):
            ...
            self['index'][name] = {'index': slice(*N), 'n': Nf[i],
