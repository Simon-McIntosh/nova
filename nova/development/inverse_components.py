...
def add_B(self, B, Bdir, factor=1, zero_norm=False, **kwargs):
    ...
    self.add_fix([x], [z], [B], np.array([[Bdir[0]], [Bdir[1]]]).T, ["Bdir"], [factor])
    if zero_norm:
        self.add_fix(
...
def get_flux(self, centre=0, width=363 / (2 * np.pi), array=[-0.5, 0, 0.5], **kwargs):
...
def inductance(self):
    ...
    turns = np.array([self.coilset["coil"][name]["Nt"] for name in self.coil["active"]])
...
def append_subcoil(self, subcoil):
    for subname in subcoil.keys():
        ...
        for key, var in zip(
            ...
            self.coil[state][name][key] = np.append(self.coil[state][name][key], var)
...
def grid_CS(self, n, Xo=2.9, Zbound=[-10, 9], dx=0.818, gap=0.1, fdr=1):
    ...
    Le = np.linspace(Zbound[0] - gap / 2, Zbound[-1] + gap / 2, nCS + 1)  # coil edges
    self.update_coils()
...
def move_PF(self, name, AR=0, **kwargs):
    ...
    if "point" in kwargs.keys() or "Lout" in kwargs.keys() or "Lin" in kwargs.keys():
        x, z = self.get_point(**kwargs)
