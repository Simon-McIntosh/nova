...
class scenario_data:
    ...
    def interpolate(self, data2, *args):
        ...
        self.interpolator = interp1d(
            self.to, data_block, axis=0, fill_value="extrapolate"
        )
        self.data2 = None
    ...
    def update(self, t, force=False):
        if (t < self.to[0] or t > self.to[-1]) and not force:
            raise IndexError(
                f"requested time: {t}s outside data range "
...
class scenario(pythonIO):
    ...
    def scenario_fault(self, M, names, dt=None, plot=False):
        ...
        for i, t in enumerate(time):
            Ic[i, :], dI[i, :], Eo[i, :], Efault[i, :] = self.fault_current(t, M, names)
    ...
    def set_limits(self):  # default limits for ITER coil-set
        ...
        self.inv.set_limit(IPF1=48, IPF2=55, IPF3=55, IPF4=55, IPF5=52, IPF6=52)
    ...
    def set_tie_plate(self):
        ...
        self.tie_plate["beta"] = np.array(
            [-0.0165, -0.0489, -0.0812, -0.113, -0.145, -0.178]
        )  # beta Fz
    ...
    def read_txt_file(self, folder, dropnan=True, force=False):
        ...
        self.data2 = self.dina.read_csv(
            filename, dropnan=True, split=",", dataframe=True
        )
        filename = self.dina.locate_file("data3.txt", folder=folder)
        self.data3 = self.dina.read_csv(
            filename, dropnan=True, split=",", dataframe=True
        )
    ...
    def update_boundary(self, eqdsk):
        limit = [[eqdsk["x"][0], eqdsk["x"][-1]], [eqdsk["z"][0], eqdsk["z"][-1]]]
        self.boundary = {
            "n": eqdsk["nx"] * eqdsk["nz"],
            "limit": limit,
            "xlim": eqdsk["xlim"],
    ...
    def loadCScurrents(self):
        ...
        for name in CSname:
            ...
            Ic_interp = interp1d(
                self.t, self.data[f"I{data_name.lower()}"], fill_value="extrapolate"
            )
    ...
    def filter_vector(self, x, dt, dt_window=1.0, polyorder=2):
        x_filter = {}
        for name in x:
            x_filter[name] = lowpass(
                x[name], dt, dt_window=dt_window, polyorder=polyorder
            )
        return x_filter
    ...
    def plot_plasma_current(self, flattop_mask=True, ax=None):
        ...
        ax.text(
            np.mean(t_ft),
            1e-6 * self.Ipl[self.flattop["index"][0]] / 2,
            txt,
            ma="left",
            ha="center",
            va="center",
    ...
    def plot_turning_points(self, n=500, read_txt=False):
        ...
        self.get_turning_points(
            "Icoil", coils=["CS"], dt_window=2.0, n=500, epsilon=50, plot=True, ax=ax[1]
        )
        self.get_turning_points(
            "Fsep",
            t=self.Fcoil["t"],
    ...
    def get_turning_points(
        self, x, dt_window=1.0, n=500, epsilon=50, plot=False, ax=None, **kwargs
    ):
        if isinstance(x, str):
            variable_name = x
    ...
    def cluster(self, t, x, turn_index, plot=False, ax=None):
        ...
        for name in turn_index:
            ...
            clusters["time"] = np.append(
                clusters["time"], clusters["t"][turn_index[name]]
            )
            clusters["name"] = np.append(
                clusters["name"], [name for __ in range(len(turn_index[name]))]
            )
    ...
    def plot_clusters(self, clusters, variable_name="", flattop_mask=True, ax=None):
        scale, ylabel = self.get_variable(variable_name)
        ft_t = self.t[self.flattop["index"]]
        ft_slice = slice(
            np.argmin(abs(ft_t[0] - clusters["t"])),
        ...
        for t_center in clusters["fit"].cluster_centers_:
            ...
            instance_vector = [
                clusters["vector"][name][index] for name in clusters["vector"]
            ]
            ylim = np.array([np.min(instance_vector), np.max(instance_vector)])
    ...
    def operate(self, plot=False, dt_window=10, nstd=3):
        ...
        self.hip = histopeaks(
            self.t[:ind], dIpldt_lp[:ind], nstd=nstd, nlim=9, nbins=300
        )  # modes
        ...
        self.flattop["dpsi"] = np.diff(self.data["PSI(axis)"][self.flattop["index"]])[0]
    ...
    def update_psi(
        self, n=None, limit=None, plot=False, ax=None, current="A", plot_nulls=False
    ):
        if n is not None:
            self.boundary["n"] = n
        if limit is not None:
            self.boundary["limit"] = limit
        x2d, z2d, x, z = geom.grid(self.boundary["n"], self.boundary["limit"])[:4]
        psi = get_coil_psi(
            x2d, z2d, self.pf.coilset["subcoil"], self.pf.coilset["plasma"]
        )
        eqdsk = {"x": x, "z": z, "psi": psi, "nx": len(x), "nz": len(z)}
    ...
    def get_flux(self, x, z):
        psi = get_coil_psi(x, z, self.pf.coilset["subcoil"], self.pf.coilset["plasma"])
        return psi

    def plot_plasma(self, ax=None, plot_nulls=False, current="A"):
        if ax is None:
            ax = plt.subplots(1, 1, figsize=(8, 10))[1]
        self.sf.contour(Xnorm=True, boundary=True, separatrix="", ax=ax)
        try:
    ...
    def load_VS3(self, n=51, plot=False):
        ...
        if plot:
            ...
            for i, VScoil in enumerate(self.VS3):
                ax[0].plot(t, self.VS3[VScoil]["Bmag"], "-C{}".format(i), label=VScoil)
                ax[1].plot(t, self.VS3[VScoil]["Bx"], "--C{}".format(i))
                ax[1].plot(t, self.VS3[VScoil]["Bz"], "-.C{}".format(i))
            ax[0].legend()
            h_x = mlines.Line2D([], [], color="gray", linestyle="--", label="$B_x$")
            h_z = mlines.Line2D([], [], color="gray", linestyle="-.", label="$B_z$")
    ...
    def calculate_Fsep(self, FzCS):
        ...
        Fsep.iloc[:, 1:-1] = np.array(
            [
                ...
                - np.sum(FzCS.iloc[:, : i + 1], axis=1)
                for i in range(np.shape(FzCS)[1] - 1)
            ]
        ).T
        Fsep.iloc[:, -1] = -Fsum
        return Fsep
    ...
    def get_max_value(self, t, df, ax=None, plot=False, unit="MN", sign=1):
        ...
        if plot:
            ...
            ax.text(time, value, f" {value:1.1f}{unit}", va=va, ha="left", color=color)
        return time, value
    ...
    def compare_force(self, Nova=False, title=False):
        ...
        self.get_max_value(
            self.post["DINA"]["t"], self.post["DINA"]["Fsep"], ax=ax[0], plot=True
        )
        ...
        self.get_max_value(
            self.post["DINA"]["t"], self.post["DINA"]["Faxial"], ax=ax[1], plot=True
        )
        ax[0].plot(
            self.post["DINA"]["t"],
        ...
        for i in range(2):
            text[i].plot()
        ax[1].legend(loc="upper center", bbox_to_anchor=(0.5, 2.7), ncol=4)
        if title:
    ...
    def compare(self):
        ...
        # text[0].plot()
        ax[0].legend(
            loc="center left", bbox_to_anchor=(1.0, 0.5), ncol=1, fontsize="xx-small"
        )
        self.get_max_value(
            self.post["DINA"]["t"],
        ...
        # text[1].plot()
        self.get_max_value(
            self.post["DINA"]["t"], Bstar.loc[::-1], ax=ax[1], plot=True, unit="T"
        )
        ax[1].plot(
            self.post["DINA"]["t"],
