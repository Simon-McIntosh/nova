...
class operate:
    ...
    def feature_extract(
        self, feature_name, eps=1e-3, dt=0.1, dt_window=None, threshold=0.95
    ):
        """
        Attributes
        ...
        xrdp = rdp_extract(self.t, y, dx=dt, eps=eps, dx_window=dt_window)[0]
        ...
        for col in self.frame.columns:
            ...
            feature["segment"].loc[:, col] = np.diff(feature["node"].loc[:, col])
        ...
        for index in feature["node"].index:
            if value.loc[index] > threshold * max_value:
                if not self.feature_switch:
                    ...
                    feature["node"].rename(index={index: tag}, inplace=True)
            elif value.loc[index] < threshold * max_value:
                if self.feature_switch:
                    tag = self.feature_tag
                    feature["node"].rename(index={index - 1: tag}, inplace=True)
                    break
            ...
            if value.loc[value.index[1]] > 0.01 * max_value:
                ...
                for index in value.index[1:]:
                    if value.loc[index] < 0.01 * max_value:
                        feature["node"].rename(index={index: "SOP"}, inplace=True)
                        break
            if "EOF" in feature["node"].index:
                EOF_index = int(feature["node"].loc["EOF", "index"])
                for index in value.index[EOF_index + 1 :]:
                    if value.loc[index] < 0.01 * max_value:
                        feature["node"].rename(index={index: "EOP"}, inplace=True)
                        break

    def extract_feature_keypoints(self):
        self.feature_keypoints = pd.DataFrame(columns=self.feature["node"].columns)
        self.feature_keypoints.rename(columns={"index": "frame_index"}, inplace=True)
        for feature_name in self._feature:
            self.feature_name = feature_name
            for index in self.feature["node"].index:
                if isinstance(index, str):
                    vector = pd.Series(self.feature["node"].loc[index, :], dtype=float)
                    vector.loc["frame_index"] = np.argmin(abs(self.t - vector.t))
                    self.feature_keypoints = self.feature_keypoints.append(vector)
        self.feature_keypoints.sort_values("t", inplace=True)

    def extract_feature_segments(self):
        self.feature_segments = pd.DataFrame(columns=["frame_index", "t", "dt", "dpsi"])
        for feature_name, label, keypoints in zip(
            ["Ip", "Pfus", "Ti", "Zx"],
            ["flattop", "burn", "hot", "Xpoint"],
            [["SOF", "EOF"], ["SOB", "EOB"], ["SOH", "EOC"], ["XPF", "XPD"]],
        ):
            self.feature_name = feature_name  # load feature
            # ensure that both keypoints are present
            if np.array([key in self.feature["node"].index for key in keypoints]).all():
    ...
    def extract_features(self):
        for variable, threshold in zip(
            ["Ip", "Pfus", "Ti", "Zx", "<PSIext>", "<PSIcoils>", "PSI(axis)"],
    ...
    def plot_features(self, feature_name=None, ax=None, ls=None):
        ...
        for i, name in enumerate(feature_name):
            ax[i].plot(self.t, self.frame.loc[:, name], alpha=0.75, color="gray")
...
class interpolate:
    ...
    def set_index(self, index=None):
        ...
        self.units = pd.DataFrame(index=self.index, columns=["read", "write", "factor"])
    ...
    def check_folder_set(self):
        if self.folder is None:
            raise NameError(
                "scenario folder unset: " "use self.load_file(folder) to load"
            )
        if self._to is None:
    ...
    def extrapolate(self, extrapolate):
        if not isinstance(extrapolate, bool):
            raise ValueError(
                f"type(extrapolate) bool != type({extrapolate}) "
                f" {type(extrapolate)})"
            )
    ...
    def check_extrapolate(self):
        if not self._extrapolate and (self._to < self.t[0] or self._to > self.t[-1]):
            raise IndexError(
                f"requested time: {self._to}s "
                "outside data range "
...
class scenario_limits:
    ...
    def initialize_limits(self):
        ...
        self.set_limit(IPF1=48, IPF2=55, IPF3=55, IPF4=55, IPF5=52, IPF6=52)
        # force limits
        self.set_limit(FCSsep=120, side="upper")
        self.set_limit(FCSsum=60, side="both")
        self.set_limit(
            FPF1=-150, FPF2=-75, FPF3=-90, FPF4=-40, FPF5=-10, FPF6=-190, side="lower"
        )
        self.set_limit(
            FPF1=110, FPF2=15, FPF3=40, FPF4=90, FPF5=160, FPF6=170, side="upper"
        )
        # field limits
    ...
    def t(self, t):
        ...
        self.frame.rename(
            columns={"Fz_1": "Fz_cssep", "Fz_2": "Fz_cssum"}, inplace=True
        )
    ...
    def plot(self, multi=True, strID="", ax=None):
        ...
        for i, index in enumerate(self.index):
            ...
            for limit in self.index[index]:
                ...
                if np.sum(max_index) > 0:
                    ...
                    if np.nanmax(_waveform) > 0.75:
                        ...
                        _label = None
                    if multi:
                        ax[i].plot(self.t, _waveform, label=_label, color=_color)
            if multi:
                # plot keypoints
                for keypoint, marker in zip(["SOF", "SOB", "EOB"], ["*", "P", "X"]):
                    try:
            ...
            ax[i].plot([self.t[0], self.t[-1]], np.ones(2), "--", color="gray")
...
class scenario_data(read_dina, interpolate, operate):
    ...
    def __init__(
        ...
        """
        read_dina.__init__(self, database_folder=database_folder, read_txt=read_txt)
    ...
    def load_file(self, folder, verbose=True, **kwargs):
        self.folder = folder
        additional_columns = kwargs.get("additional_columns", self.additional_columns)
        if self.folder is not None:
            ...
            if read_txt or not isfile(filename + ".pk"):
                ...
                if self.load_pickle(filename) or (
                    sorted(additional_columns) != self.additional_columns
                ):
                    self.read_file(folder, additional_columns)
                    self.save_pickle(filename, attributes)
    ...
    def rename_columns(self):
        ...
        self.data.rename(
            columns={"Rsep_or_Rlim": "Rx_or_Rtouch", "Zsep_or_Zlim": "Zx_or_Ztouch"},
            level=0,
            inplace=True,
        )
        self.data.rename(
            columns={"Rx_or_Rtouch": "Rx", "Zx_or_Ztouch": "Zx"}, level=0, inplace=True
        )
    ...
    def subindex(self, additional_columns):
        ...
        index = [
            var
            for var in self.data.columns.droplevel(1)
    ...
    def build_index(self, additional_columns):
        ...
        coil_names, self.Ic_iloc = np.array(
            [[v, i] for i, v in enumerate(self.index) if ("I" in v and len(v) <= 5)]
        ).T  # and v != 'Ip'
        ...
        self.Ic_iloc = np.insert(
            self.Ic_iloc, cs1_loc, [self.Ic_iloc[cs1_loc], self.Ic_iloc[cs1_loc]]
        )
    ...
    def interpolate(self, data, additional_columns=[]):
        ...
        self.interpolator = interp1d(t, data_block, axis=0, fill_value="extrapolate")
        self.data = None  # unlink raw data (reload from source)
        self.frame = pd.DataFrame(self.interpolator(self.t), columns=self.index)
    ...
    def plot(
        ...
        **kwargs,
        ...
        if xslice is None:
            ...
            for i in range(len(xslice)):
                if isinstance(xslice[i], str):
                    xslice[i] = self.feature_keypoints.loc[xslice[i], "frame_index"]
        if len(xslice) == 1:
            dt = 0
            xslice = xslice[0]
        else:
            dt = self.frame.loc[xslice[1], "t"] - self.frame.loc[xslice[0], "t"]
            xslice = slice(*xslice)
        ...
        if isinstance(x, float):
            ...
            if dt > dt_min:
                strID = strID.replace(" ", "_")
                label = kwargs.pop("label", self.filename.replace(strID, ""))
...
class forcefield_data(read_dina, interpolate):
    ...
    def load_file(self, folder, verbose=True, **kwargs):
        ...
        if self.folder is not None:
            ...
            attributes = ["t", "dt", "_to", "interpolator", "frame", "_vector", "index"]
            if read_txt or not isfile(filename + ".pk"):
...
class read_scenario(read_dina):
    def __init__(
        self, folder=None, database_folder="scenarios", read_txt=False, file_type="txt"
    ):
    ...
    def read_txt_file(self, folder, dropnan=True):
        ...
        self.data2 = self.read_csv(filename, dropnan=dropnan, split=",", dataframe=True)
        filename = self.locate_file("*data3.txt", folder=folder)
        self.data3 = self.read_csv(filename, dropnan=dropnan, split=",", dataframe=True)

    def read_qda_file(self, folder, dropnan=True):
        filename = self.locate_file("*data2.qda", folder=folder)
        self.data2 = self.read_qda(filename, dropnan=dropnan, split=",", dataframe=True)
        filename = self.locate_file("*data3.qda", folder=folder)
        self.data3 = self.read_qda(filename, dropnan=dropnan, split=",", dataframe=True)
