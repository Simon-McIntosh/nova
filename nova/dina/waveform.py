...
class WaveForm:
    ...
    def _extract_source(self):
        ...
        source_columns = ["t", "Rp", "ap", "kp", "li(3)", "BETAp", "Rcur", "Zcur"]
        current_iloc = np.unique(scenario.Ic_iloc)
        source_columns.extend(
            [label for label in scenario.index[current_iloc] if "tf" not in label]
        )
    ...
    def _add_control_currents(self, loop):
        ...
        if loop_label not in self.source:
            raise IndexError(
                f"loop label {loop_label} " f"not found in {self.source.columns}"
            )
        ...
        for coil, factor in zip(coils, factors):
            self.source.loc[:, coil] = self.source.loc[:, coil] + factor * loop_current
    ...
    def plot_spectrum(
        self, substring, attr="source", axes=None, legend=False, title=False, **kwargs
    ):
        """
        ...
        if title:
            plt.title(f"{self.scenario}\n{attr}: " f'fs={dataframe.attrs["fs"]:1.1f}Hz')
