...
class CrossSection:
    ...
    skin_fraction: float = 1


@dataclass
class CoilFrame(Mesh, CrossSection):
    """Mesh coils."""
    ...
    def mesh_poloidal(self, index=None, link=True, **kwargs):
        """
        Mesh coil(s). Store filaments in subcoil. Link turns.
        ...
        **kwargs : dict
        ...
        for i, name in enumerate(index):
            ...
            mesh = self._mesh_poloidal(frame.loc[name, :], link=link,
                                       **kwargs)  # single coil
...
if __name__ == '__main__':
    ...
    coilframe = CoilFrame(dpol=0.1)
    coilframe.add_poloidal(range(3), 1, 0.1, 0.1, link=True, delta=3)
    print(coilframe.frame.delta)
