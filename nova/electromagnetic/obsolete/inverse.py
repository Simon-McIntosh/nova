...
class INV(object):
    ...
    def get_boundary(self, npoint=21, alpha=0.995):
        try:
            ...
            Xindex = np.argmin((x-self.sf.Xpoint[0])**2 + 
                               (z-self.sf.Xpoint[1])**2)
            x = np.append(x[Xindex:], x[:Xindex])
            z = np.append(z[Xindex:], z[:Xindex])
            Xpoint = True
        except TypeError:  # X-point not present
            x = np.array([self.sf.x[0], self.sf.x[-1], 
                          self.sf.x[-1], self.sf.x[0], self.sf.x[0]])
            z = np.array([self.sf.z[0], self.sf.z[0], 
                          self.sf.z[-1], self.sf.z[-1], self.sf.z[0]])    
