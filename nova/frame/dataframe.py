...
class DataFrame(FrameAttrs):
    ...
    def update_columns(self):
        ...
        with self.setlock(None):
            ...
            # check required
            required_unset = [attr not in columns for attr in self.metaframe.required]
            ...
            if frame.isna().values.any():
                for attr in frame:
                    ...
                    if isna.any() and not any(
                        [value is None for value in frame[attr].values]
                    ):
                        self.loc[isna, attr] = self.metaframe.default[attr]
            # extend additional
            additional = [
                attr for attr in columns if attr not in self.metaframe.columns
            ]
            if additional:
                self.metaframe.metadata = {"additional": additional}
            # set defaults
            additional_unset = [attr not in columns for attr in self.metaframe.columns]
            if np.array(additional_unset).any():
                ...
                turn_set = np.array([attr in self.columns for attr in ["It", "nturn"]])
                if "Ic" in unset and turn_set.all():
                    self.loc[:, "Ic"] = self.loc[:, "It"] / self.loc[:, "nturn"]
    ...
    def _loads(self, col: str):
        ...
        geotype = [json.loads(geom)["type"] if geom else None for geom in self[col]]
        self.loc[:, col] = [
            self.geoframe(geo).loads(geom) for geom, geo in zip(self[col], geotype)
        ]
    ...
    def _astype(self, metadata: dict[str, list[str]], dtype: str) -> dict:
        ...
        return {
            key: np.array(value, dtype=dtype).tolist()
