...
from nova.frame.dataarray import ArrayLocMixin, ArrayIndexer, DataArray
...
class FrameLink(LinkIndexer, DataArray):
    ...
    def _extract_frame(self, *args, **kwargs):
        ...
        **kwargs : dict[str, Union[float, array-like]]
        ...
        if len(args) != 1:
            args += tuple(
                kwargs.pop(arg) for arg in self.metaframe.required if arg in kwargs
            )
            return args, kwargs
    ...
    def _patch_current(self, data, attrs=None):
        ...
        if "It" in attrs and "Ic" not in attrs:
            data["Ic"] = data["It"] / data.get("nturn", self.metaframe.default["nturn"])
        elif "It" in attrs and "Ic" in attrs:
            data["It"] = data["Ic"] * data.get("nturn", self.metaframe.default["nturn"])
