...
class MachineData(CoilSet, pythonIO):
    ...
    def orient(frame):
        ...
        if frame.shape[0] > 2 and "x" in frame and "z" in frame:
            ring = shapely.geometry.LinearRing(frame.loc[:, ["x", "z"]].values)
            if not ring.is_ccw:
                frame = frame.iloc[::-1, :]
        return frame
    ...
    def read_sheet(self, sheetname, skiprows, usecols, columns={}, nrows=None):
        ...
        sheet = pd.read_excel(
            self.f, sheetname, skiprows=skiprows, usecols=usecols, nrows=nrows
        ).dropna()
        columns = {**{"R, m": "x", "Z, m": "z"}, **columns}
        if np.array(
            [True for c in sheet.columns if ".1" in c or ".2" in c or "eff" in c]
        ).any():
            strip = {
                c: c.replace(".1", "").replace(".2", "").replace("eff", "")
    ...
    def read_models(self):
        """Read model set."""
        self.models = {}
        self.filename = "Models_for_calculation_of_axisymmetric_c_XBQF5H_v2_2.xlsx"
        with pd.ExcelFile(
            path.join(self.directory, self.filename), engine="openpyxl"
        ) as self.f:
            self.read_model(
    ...
    def read_data(self):
        """Read geometric data."""
        self.data = {}
        self.filename = "Data_for_study_of_ITER_plasma_magnetic_c_33NHXN_v3_15.xlsx"
        with pd.ExcelFile(
            path.join(self.directory, self.filename), engine="openpyxl"
        ) as self.f:
            ...
            self.data["cryostat"] = self.read_sheet(
                "Cryostat & CST", 8, np.arange(1, 5)
            )[:14]

            self.data["cryostatCTS"] = self.read_sheet(
                "Cryostat & CST", 8, np.arange(1, 5)
            )[13:]
