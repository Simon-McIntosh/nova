...
class MetaData(ABC):
    ...
    def metadata(self):
        ...
        return {
            ...
            for field in fields(self)
            if field.name not in self._internal
        }
            ...
            if len(value) == 0:  # [], None or False
                ...
                if replace:
                    ...
                    getattr(self, attribute).extend(
                        [
                            attr
                            for attr in list(dict.fromkeys(value))
                            if attr not in getattr(self, attribute)
                        ]
                    )
                else:
                    raise TypeError(
                        f"attribute type {types[attribute]} ", "not in [list, dict]"
                    )
        self.validate()
    ...
    def validate(self):
        ...
        types = self.types
        type_error = {
            field.name: [types[field.name], type(getattr(self, field.name))]
            for field in fields(self)
            if not isinstance(getattr(self, field.name), types[field.name])
        }
        if type_error:
            raise TypeError(
