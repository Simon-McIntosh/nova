...
class VDE(Axes, CoilSet):  # read_dina,
    ...
    def set_path(self, folder):
        """Set full filepath."""
        try:
            self.dina_file = read_dina("disruptions").locate_file(
                "tor_cur", folder=folder
            )
    ...
    def insert_shells(self, dt=60e-3):
        ...
        for i in range(self.nf):
            ...
            for j in range(n_turn):
                x1, z1, x2, z2, turn = [float(d) for d in self.rt.readline(True)]
                for var, value in zip(
                    ["x1", "z1", "x2", "z2", "turn"], [x1, z1, x2, z2, turn]
                ):
        ...
        for i, filament in enumerate(self.filaments):
            for j in range(filament["n_turn"]):
                ...
                self.shell.insert([x1, x2], [z1, z2], 0, dt, label=part, part=part)
    ...
    def read_data(self, transient):
        ...
        self.data = xarray.Dataset(
            coords=dict(
                index=self.subframe.subspace.index.to_list(),
                plasma=self.subframe.index[plasma].to_list(),
                time=time,
            )
        )
        self.data["It"] = xarray.DataArray(
            0.0, dims=["time", "index"], coords=[self.data.time, self.data.index]
        )
        self.data["Ic"] = xarray.DataArray(
            0.0, dims=["time", "index"], coords=[self.data.time, self.data.index]
        )
        self.data["nturn"] = xarray.DataArray(
            0.0, dims=["time", "plasma"], coords=[self.data.time, self.data.plasma]
        )
        nturn = self.frame.loc[self.frame.subspace.index, "nturn"]
        for i, frame in enumerate(transient):
            ...
            turns = scipy.interpolate.griddata(
                (xp, zp), Ip, self.loc["ionize", ["x", "z"]], method="nearest"
            )
    ...
    def plot_waveform(self, index=slice(1, -10), axes=None, filt=True, **kwargs):
        """Plot probe waveforms."""
        if axes is None:
            axes = plt.gca()
        if filt:
            time, bz = self.filt(
                self.probe.data.time[index], self.probe.data.bz[index, 0], 25
            )
            ...
            return dbdt_z
        plt.plot(
            self.probe.data.time[index], self.probe.data.dbdt_z[index, 0], **kwargs
        )
    ...
    def filt(self, time, value, cutoff):
        ...
        _time = np.linspace(time[0], time[-1], int((time[-1] - time[0]) / dt_min))
...
if __name__ == "__main__":
    ...
    def make_frame(position, axes=None):
        ...
        vde.grid.plot(axes=vde.axes, levels=20, colors="darkgray", linewidths=2)
        # plot duck
        duck.update(index)
        duck.grid.plot(
            axes=vde.axes,
    ...
    def plot_currents():
        ...
        plt.figure()
        plt.bar(
            range(duck.loc["free"].sum()),
            1e-3 * vde.loc[duck.loc["free"], "It"],
            label="DINA",
        )
        plt.bar(
            range(duck.loc["free"].sum()),
            1e-3 * duck.loc["free", "It"],
            alpha=0.5,
            label="Duck",
        )
        plt.despine()
