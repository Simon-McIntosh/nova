...
class Magnetics(Plot, Database):
    ...
    user: str = 'public'
    machine: str = 'iter_md'
    data: dict[str, pandas.DataFrame] = field(init=False, repr=False,
                                              default_factory=dict)

    signal: ClassVar[dict[str, str]] = dict(
        A1='i', A2='i', A3='i', A4='i', A5='p', A6='p', A7='i', A8='p', A9='i',
        AA='i', AB='i', AC='i', AD='i', AE='i', AF='i', AG='i', Ah='i', AI='i',
        AJ='i', AK='i', Al='i', AM='i', AN='i', AO='i', AP='i')
    ...
    def build_frame(self):
        ...
        for diagnostic in self.diagnostic:
            ...
            frame = pandas.DataFrame(dict(name=name,
    ...
    def build_summary(self):
        ...
        self.data['summary'] = pandas.DataFrame(
            dict(name=name, identifier=identifier, diagnostic=diagnostic_type,
    ...
    def build_flux_loops(self):
        ...
        for ids in self.get_ids('flux_loop'):
            ...
            data.append(
                [ids.name, ids.identifier, group, ids.type.index,
                 *[np.array([getattr(position, attr)
                             for position in ids.position])
                   for attr in ['r', 'z', 'phi']],
                 ids.indices_differential, ids.area, ids.gm9])
        self.data['flux_loop'] = pandas.DataFrame(
            data, columns=['name', 'identifier', 'group', 'type',
                           'r', 'z', 'phi', 'indices', 'area', 'gm9'])
    ...
    def plot(self, axes=None):
        ...
        for index in data.loc[data.group == 'AD'].index:
                ...
                self.axes.plot(data.loc[index, 'phi'],
                               data.loc[index, 'z'], 'o-')
