...
class PulseSchedule(Plot, Scenario):
    ...
    def time_coordinate(self, path: str, attr: str, index=None):
        ...
        if index is None:
            ...
            self.data.coords[coord] = self.ids_index.get_slice(index, path + ".time")
        return (coord,)
    ...
    def build_point(self, ids_node: str):
        ...
        with self.ids_index.node(ids_node):
            if self.ids_index.empty("r.reference.data") and self.ids_index.empty(
                "z.reference.data"
            ):
                return
    ...
    def _angle(self, corner_eps=0.01):
        ...
        angle[2::3] = np.mean(np.c_[panel_angle[:-1], panel_angle[1:]], axis=1)
    ...
    def build(self):
        ...
        with self.build_scenario():
            self.data.coords["point"] = ["r", "z"]
            self.build_0d(
                "flux_control", ["i_plasma", "loop_voltage", "li_3", "beta_normal"]
            )
            self.build_0d(
                "position_control",
                [
    ...
    def plot_profile(self):
        ...
        self.axes[3].plot(self.data.time, self.data.minor_radius, "C3", label=r"$r$")
        self.axes[3].plot(self.data.time, self.data.elongation, "C4", label=r"$\kappa$")
        self.axes[3].plot(
            self.data.time, self.data.triangularity, "C5", label=r"$\delta$"
        )
    ...
    def plot_gaps(self):
        ...
        arrows = [
            ...
            for x, z, dx, dz in zip(tail[:, 0], tail[:, 1], vector[:, 0], vector[:, 1])
        ]
