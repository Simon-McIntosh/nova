...
class Waveform(Animate, MachineDescription, PulseProfile):
    ...
    def solve_biot(self):
        ...
        self.plasmagap.solve(
            self.data.gap_tail.data, self.data.gap_angle.data, self.data.gap_id.data
        )
    ...
    def _update_loop_psi(self):
        ...
        self.sloc["coil", "Ic"] = np.linalg.lstsq(
            Psi[:, self.sloc["coil"]], loop_psi - plasma_psi, rcond=None
        )[0]
    ...
    def _make_frame(self, time):
        ...
        try:
            self.itime = (
                bisect.bisect_left(self.data.time, max_time * time / self.duration)
                + 1902
            )
        except ValueError:
            pass
