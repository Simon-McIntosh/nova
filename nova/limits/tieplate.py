...
def get_tie_plate(wp="Soft", link=False):
    ...
    if wp == "Ansys_old":
        ...
        beta = np.array([0.0165, 0.0489, 0.0812, 0.113, 0.145, 0.178])
        gamma = 2.95e-2
    elif wp == "Stiff":
        alpha = -0.0025
        beta = np.array([0.0254, 0.0752, 0.1249, 0.1737, 0.2235, 0.2733])
        gamma = 0.0455
    elif wp == "CSM1":
        alpha = -9.9e-3
        beta = np.array([0.0229, 0.0681, 0.113, 0.158, 0.203, 0.248])
        gamma = 4.27e-2
    elif wp == "FEniCs":
        alpha = -9.37e-3
        beta = np.array([0.0218, 0.0647, 0.108, 0.15, 0.193, 0.236])
        gamma = 4.27e-2
    elif wp == "Soft":
        alpha = -0.0018
        beta = np.array([0.0373, 0.1111, 0.1849, 0.2580, 0.3318, 0.4056])
        gamma = 0.0704
        tie_plate["preload"] = 207.8
    elif wp == "Built":
        alpha = -0.0019
        beta = np.array([0.0389, 0.1161, 0.1933, 0.2696, 0.3468, 0.4239])
        gamma = 0.0739
