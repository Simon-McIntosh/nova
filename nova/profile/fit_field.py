...
class plasmafoil():
    ...
    def set_plasma(self, Ipl=-15e6, x=6.22, z=0.49,
                   a=None, kappa=None, beta=None, li=None):
        plasma_parameters = {'Ipl': Ipl, 'xcur': x, 'zcur': z, 'a': a,
                             'kappa': kappa, 'beta': beta, 'li': li}
    ...
    def polygen(self, segment, *args):
        ...
        bounds = [(0, 1), (0, 1), (0, 1),
                  (fLf*fL, fL), (fLf*fL, fL), (fLf*fL, fL),
                  (-dft, dft), (-dft, dft), (-dft, dft)]
        for arg in args:  # constrain variables
            bounds[arg[0]] = (arg[1], arg[1])
        args = (self.pb, self.construct, self.bndry[segment],
                self.bndry['loop']['Btmax'])
        x = minimize(self.polyfit, xo, args=args, method='L-BFGS-B',
                     bounds=bounds).x
        self.polyfit(x, *args)
        print(segment, x)
    ...
    def link(self, label):
        for var in ['ln', 'Bt', 'L']:
            self.profile[label]['loop'][var] = \
                np.append(self.profile[label]['inner'][var],
                          self.profile[label]['outer'][var])
    ...
    def plot_profiles(self, label='fit'):
        ...
        ax[0].plot(self.profile[label]['loop']['L'],
                   self.profile[label]['loop']['Bt'])
        for segment in ['inner', 'outer']:
            ax[0].plot(self.profile[label][segment]['L'],
                       self.profile[label][segment]['Bt'], '--', color='C3')
            ax[1].plot(
                    self.bndry[segment]['ln'],
                    self.bndry[segment]['Bt'] / self.bndry['loop']['Btmax'],
                    'C0-')
            ax[1].plot(self.profile[label][segment]['ln'],
                       self.profile[label][segment]['Bn'], '--', color='C3')
    ...
    def get_plasma_current(self, label):
        ...
        Ip = np.trapz(self.profile[label]['loop']['Bt'],
                      self.profile[label]['loop']['L']) / cc.mu_o
        return Ip
...
class loop:
    ...
    def bounding_box(self):
        self.bb = [np.max(self.x) - np.min(self.x),
                   np.max(self.z) - np.min(self.z)]
    ...
    def close(self, eps=1e-3):  # close loop
        if np.sqrt((self.x[0] - self.x[-1])**2 + (self.z[0] - self.z[-1])**2) \
                > eps * np.max(self.bb):
    ...
    def update_field(self, Bspline):
        self.B = {'x': Bspline[0].ev(self.x, self.z),
                  'z': Bspline[1].ev(self.x, self.z)}
        self.B['t'] = np.array(
                [np.dot(b, t) for b, t in
                 zip(np.array([self.B['x'], self.B['z']]).T, self.that.T)])
    ...
    def extract_boundary(self, segment):
        ...
        bndry['Lln'] = interp1d(bndry['ln'], bndry['L'],
                                fill_value='extrapolate')
        bndry['Lz'] = interp1d(bndry['z'], bndry['L'],
                               fill_value='extrapolate')
    ...
    def set_bound(self, bndry, variable):
        bndry[f'{variable}bound'] = np.array([bndry[variable][0],
                                              bndry[variable][-1]])
        bndry[f'{variable}lim'] = np.array([np.min(bndry[variable]),
                                            np.max(bndry[variable])])
