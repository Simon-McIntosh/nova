...
class cold_test(pythonIO):
    _ID = {"TCS": "TCS", "DS": "displace", "SG": "strain", "TC": "TCS", "EX": "extend"}
    _units = {
        "%Open": "valve",
        "kA": "current",
        "K": "temperature",
    ...
    def read_strain_file(self, folder, filename):
        ...
        elapsed = pandas.read_csv(filepath, skiprows=6, usecols=[2])
        columns = {}
    ...
    def fit(
        self, label, index="test", Imin=7.5, Imax=40, plot=True, ncol=2, color=None
    ):
        index = self.get_index(index)
        # extract current
        ...
        if plot:
            ...
            if group == "strain":
                ...
                value = "1.2f"
            text = linelabel(
                value=value, postfix=self._units_r.get(group, "mm"), Ndiv=20
            )
            for i, col in enumerate(dataframe):
                if color is None:
                    c = f"C{i}"
...
if __name__ == "__main__":
    ...
    ct.fit(
        ["EX-170-ID", "EX-270-OD", "EX-4310-OD"],
        "comp",
        ncol=3,
        color=["C0", "C1", "C3"],
    )
    ct.fit(["DS001", "DS002", "DS004"], "comp", ncol=3, color=["C0", "C1", "C3"])
