...
class Material_Model:
    ...
    def set_data(self):
        ...
        self.data = dict(IDdv=-2.05, ODdv=-1.19, IDh0=788, ODh0=437)  # 40kA CSM1
        """
    ...
    def material_model_name(self, name):
        if len(name) == 1 and isinstance(name, str):
            ...
            if name in _m:
                ...
                raise IndexError(
                    f"quick index {name} " + f"not found in _models {self._models}"
                )
        ...
        if self.material_model is not None:
            if self.material_model == "isotropic":  # isotropic coefficients
                ...
                self.material_data = {
                    ...
                    "nu_tp": 0.29,
                }  # toroidal-poloidal Poisson's ratio
                self._ismodulus[:2] = True
            elif self.material_model == "orthotropic":
                "6 free variables"
                self._set_attributes(
                    ["Ex", "Et", "Ez", "Gxz", "nu_xz", "nu_tx", "nu_tz"], ["C"]
                )
                ...
                raise NotImplementedError(
                    f"material_model {self.material_model} " + "not implemented"
                )
    ...
    def _update_isotropic(self):
        ...
        self.fenics_data = {"mu": df.Constant(mu), "lmbda": df.Constant(lmbda)}
    ...
    def _raise_data_error(self, attribute, attribute_type):
        ...
        raise IndexError(
            f"attribute {attribute} incompatable "
...
class CSmodulue(Material_Model):
    def __init__(
        self, read_txt=False, material_model="isotropic", num=None, labels=None
    ):
    ...
    def _assemble_function_space(self):
        ...
        self.T = df.TensorFunctionSpace(self.mesh, "CG", degree=2, shape=(3, 3))
    ...
    def _assemble_internal_work(self):
        self.a = df.inner(self.sigma(self.du), self.eps_u_) * self.x[0] * self.dx

    def constrain(self):
        return [
            df.DirichletBC(
                self.V.sub(1), df.Constant(0), Edge(self.limit), method="pointwise"
            )
        ]
    ...
    def _update_force_interpolators(self):
        J = self.coilset.subframe.It.sum() / (
            self.coilset.frame.dx[0] * self.coilset.frame.dz[0]
        )

        Psi = np.dot(self.biotgrid.data.Psi, self.coilset.sloc["Ic"])
        Psi.shape = self.biotgrid.shape
        psi_x, psi_z = np.gradient(
            Psi / (2 * np.pi), self.biotgrid.data.x, self.biotgrid.data.z
        )
        xm = self.biotgrid.data.x2d.values
        xm[xm == 0] = 1e-34
        Bx = -psi_z / xm
        Bz = psi_x / xm
        self.fx = RectBivariateSpline(
            self.biotgrid.data.x, self.biotgrid.data.z, J * Bz
        )
        self.fz = RectBivariateSpline(
            self.biotgrid.data.x, self.biotgrid.data.z, -J * Bx
        )
    ...
    def eps(self, v):  # axisymetric strain tensor
        return df.sym(
            df.as_tensor(
                [
                    [v[0].dx(0), 0, (v[0].dx(1) + v[1].dx(0)) / 2],
                    [0, v[0] / self.x[0], 0],
    ...
    def voigt2stress(s):
        ...
        s = [sx, st, sz, sxz]
        """
        return df.as_tensor([[s[0], 0, s[3]], [0, s[1], 0], [s[3], 0, s[2]]])
    ...
    def sigma_orthotropic(self, v):
        return self.voigt2stress(df.dot(self.C, self.strain2voigt(self.eps(v))))
    ...
    def extract_displacments(self, verbose=True):
        ...
        self.position = dict(
            IDdv=(self.limit[::3], self.limit[::2]),
        ...
        displace = {}
        displace[("IDdv", "mm")] = 1e3 * (
            self.u(self.position["IDdv"][0])[1] - self.u(self.position["IDdv"][1])[1]
        )
        displace[("ODdv", "mm")] = 1e3 * (
            self.u(self.position["ODdv"][0])[1] - self.u(self.position["ODdv"][1])[1]
        )
        # displace[('Udr', 'mm')] = 1e3 * (
        ...
        if verbose:
            ...
            dataframe = pandas.DataFrame(
                index=displace.index, columns=["unit", "value", "FEA", "fit", "error %"]
            )
            dataframe.loc[:, displace.columns] = displace
            dataframe.loc[self.data.keys(), "value"] = self.data
            dataframe["fit"] = [name in self.labels for name in dataframe.index]
            dataframe["error %"] = (
                1e2 * (dataframe.FEA - dataframe.value) / dataframe.value
            )
    ...
    def material_data_text(self, material_data=None, full=False, tex=False):
        ...
        modulus_text = ", ".join(
            (
                rf"{attr}={1e-9*M:1.1f}"
                for attr, M in zip(attributes[_ismodulus], material_data[_ismodulus])
            )
        )
        nu_text = ", ".join(
            (
                rf"{attr}={nu:1.3f}"
                for attr, nu in zip(attributes[~_ismodulus], material_data[~_ismodulus])
            )
        )
    ...
    def extract_properties(self, labels=None):
        ...
        x = minimize(
            ...
            args=data,  # , 323, 455
            method="SLSQP",
            options={"ftol": 1e-5},  # 1e-4
            bounds=(*[Mlim for __ in range(nM)], *[Nulim for __ in range(nNu)]),
        )
        print(x)
    ...
    def plot(self, scale=100, full=False, twin=False):
        ...
        if twin:
            ...
            self.biotgrid.plot(axes=ax[0], colors="gray", linewidths=1.5, zorder=-50)
            plt.despine()
            plt.axis("off")
            ax_disp = ax[1]
        else:
