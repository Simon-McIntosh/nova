...
class fluxmap(pythonIO):
    ...
    def package_Hmode_pedestal(self):
        points = self.load_points("ITER_PF_15MA_with_pedestal", date="2020_06_17")
        self.limits = {}
        self.limits["access"] = {
            "Li3": points[6]["x"],
            "Psi": points[6]["y"],
        ...
        Psi = interp1d(
            self.limits["access"]["Li3"][:-1],
            self.limits["access"]["Psi"][:-1],
            fill_value="extrapolate",
        )
        self.limits["access"]["Li3"] = np.append(self.limits["access"]["Li3"], 0.65)
        ...
        access_points = [
            (Li3, Psi)
            for Li3, Psi in zip(
                self.limits["access"]["Li3"], self.limits["access"]["Psi"]
            )
        ]
        ...
        dome_points = [
            (Li3, Psi)
            for Li3, Psi in zip(
                self.limits["low Li SOB"]["Li3"], self.limits["low Li SOB"]["Psi"]
            )
        ]
    ...
    def plot_limits(self, ax=None):
        ...
        for limit in self.limits:
            if self.limits[limit]["limit"] is not None:
                ...
                ax.plot(
                    self.limits[limit][xvar],
                    self.limits[limit][yvar],
                    ls,
                    label=label,
                    lw=lw,
                    alpha=alpha,
                ...
                if limit == "access":
                    labelLine(
                        ...
                        align=True,
                        fontsize="xx-small",
                        bbox={"facecolor": "lightgray", "ec": None, "lw": 0},
                    )
