...
if config is 'DD3':
    ...
    rb.rzGet()  # VV ports
    rb.fill(trim=[0.795, 0.795 + 0.04], dt=VV,
            ref_o=3 / 8 * np.pi, alpha=1, color='r')
    rb.rzGet()
    ...
    rb.TFsupport(dt=4 * dRsteel + dRcoil,
                 trim=[0.344, 0.344 + 0.03], color='k', alpha=1)
    rb.TFsupport(dt=4 * dRsteel + dRcoil,
                 trim=[0.418, 0.418 + 0.03], color='k', alpha=1)
...
rb.target_fill(graze, dPlate, dt=0.1, alpha=0.7,
               color='c', trim=None)  # targets
rb.sol.plot()
...
line, = pl.plot([0, 0], [-7, 6], 'r-', linewidth=2)
line.set_dashes(3 * np.array([8, 4, 2, 4]))

pl.axis('off')
pl.savefig('../Figs/FEC_vessel_double.png', dpi=900)
