...
class DINA_Blanket(FilePath):
    ...
    def mat_data(self, imas=False):
        ...
        radius, height, length_alpha, length_beta, alpha, beta = \
            data.loc[:, ['rc', 'zc', 'dl', 'dh', 'alpha', 'beta']].values.T

        if imas:
            # apply corrections to DINA .mat data
            alpha -= np.pi/2
            radius += 0.5*(length_alpha * np.cos(beta) -
                           length_beta * np.sin(alpha))
            height += 0.5*(length_alpha * np.sin(beta) +
            ...
            data['coordinates'] = [self.oblique_patch(*coords) for coords in
                                   zip(radius, height, length_alpha,
                                       length_beta, alpha, beta)]
            return data
        coordinates = self.DINA_patch(radius, height, length_alpha,
                                      length_beta, alpha, beta)
    ...
    @staticmethod
    def DINA_patch(radius, height, length_alpha, length_beta,
                   alpha, beta):
        """Return skewed polygon - translation from vessel_runs_plot.m."""
        coordinates = np.zeros((len(radius), 4, 2))

        coordinates[:, 0, 0] = radius - 0.5*(length_alpha * np.cos(beta) +
                                             length_beta * np.cos(alpha))
        coordinates[:, 0, 1] = height - 0.5*(length_alpha * np.sin(beta) +
                                             length_beta * np.sin(alpha))
