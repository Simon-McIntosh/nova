...
class MultiFilament:
        ...
        with readtxt(filename) as file:
            while True:
                try:
                    ...
                    break
        self.plasma = pandas.DataFrame(index=range(filament_number),
                                       columns=['x', 'z', 'dx', 'dz', 'Ip'])
    ...
    def calc(self):
        self.x_current =\
            (sum(self.plasma.x**2*self.plasma.Ip) / self.plasma.Ip.sum())**0.5
    ...
    def plot(self):
        ...
        plt.plot(self.x_current, self.z_current, 'C3o',
        ...
        plt.plot(x_box, z_box, 'ks',
                 label=f'bbox ({x_box:1.2f}, {z_box:1.2f})')
        plt.legend()
