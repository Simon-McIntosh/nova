...
class VDE_force(pythonIO):
    def __init__(self, mode="control", discharge="DINA", Ip_scale=1, read_txt=False):
    ...
    def load_vs3(self, folder, discharge="DINA"):
        self.Ivs3_fun = self.pl.Ivs3_single(folder, discharge=discharge)[
            -1
    ...
    def set_coil_current(self, frame_index):  # PF / Cs
        Ic = dict(zip(self.tor.coil.keys(), self.tor.current["coil"][frame_index]))
    ...
    def datafile(self, discharge, nframe):
        filename = join(
            self.dina.root, "DINA/Data/vs3_{}_{:d}.plk".format(discharge, nframe)
        )
        if self.Ip_scale != 1:
    ...
    def plot_Fmax(self, discharge, nframe=100):
        ...
        h = []
        for mode, color in zip(["referance", "control", "error"], ["gray", "C0", "C3"]):
    ...
    def plot_line_force(self, vs3, plot_full):
        ...
        for i, k in enumerate([1, 0]):
            for mode, color in zip(
                ["referance", "control", "error"], ["gray", "C0", "C3"]
            ):
        ...
        ax[1].set_xlabel("$t$ ms")
        h = []
        h.append(mlines.Line2D([], [], ls="-", color=0.3 * np.ones(3), label="LTC"))
        if plot_full:
    ...
    def plot_current(self, vs3, plot_full, file):
        ...
        for mode, color in zip(["referance", "control", "error"], ["gray", "C0", "C3"]):
            if mode == "control" or plot_full:
                ax.plot(1e3 * vs3[mode]["t"], 1e-3 * vs3[mode]["I"], color=color)
        plt.despine()
        ax.set_ylabel("$I$ kA")
        ax.set_xlabel("$t$ ms")
        path = os.path.join(class_dir(nep), "../Data/LTC/")
        ...
        for i, k in enumerate([1, 0]):
            for mode, color in zip(
                ...
                if mode == "control" or plot_full:
                    ...
                    ax[i].plot(1e3 * vs3[mode]["t"][index], Bmag, "-.", color=color)
                    ax[i].plot(
                        1e3 * vs3[mode]["t"][index],
                        vs3[mode]["Bmag"][index, k],
                        "-",
                        color=color,
                    )
        plt.despine()
