...
class Force:
    ...
    def calculate(self, plot=False):
        (
            B,
            F,
        ) = self.psi.get_force_array()  # load point values
        Ftn = np.zeros((self.nt, 3, self.vs.nP))  # rotate force vector
        for i in range(self.vs.nP):
            Ftn[:, :, i] = qrotate(F[:, :, i], theta=-self.vs.theta[i], dx=[0, 1, 0])
        ...
        if plot:
            ...
            for i, coil in enumerate(self.vs.geom):
                ax[i + 1].plot(
                    self.t,
                    np.linalg.norm(1e-3 * Ftn[:, :, i], axis=1),
                    "C7",
                    label=r"$|\bfF\rm|$",
                )
    ...
    def calculate_max(self):
        ...
        x = range(len(folders))
        for i, coil in enumerate(self.vs.geom):
            ax[i].bar(x, Fo_max[:, i], width=0.8, color="C3", label=r"$|\bfF_o\rm|$")
            ax[i].bar(x, F_max[:, i], width=0.6, color="C7", label=r"$|\bfF\rm|$")
