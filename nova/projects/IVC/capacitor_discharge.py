...
class power_supply(pythonIO):
    ...
    def build_coils(self, plot=False):
        ...
        self.ind.add_coil(
            OrderedDict(list(self.coilset["subcoil"].items())[:8]), turns=turns
        )
    ...
    def load_background_flux(self):
        if self.scenario < 0:  # set zero flux
            ...
            self.cf.load_file(self.scenario, plot=False, vessel_model=self.vessel_model)
    ...
    def check_setpoint(self, t, Iode):  # attained set-point
        if len(self.data["dt_rise"]) <= self.data["npulse"] and self.op(
            "ge", Iode[2], 0.9 * self.Itrip
        ):
    ...
    def set_switches(self, t, Iode):
        if self.capacitor_discharge:  # capacitor discharging
            dI = self.dIdt(t, Iode)
            if self.op("ge", Iode[2], 0.1 * self.Itrip) and self.tc10 <= self.tco:
                self.tc10 = t  # 10% capacitor current rise
            self.check_setpoint(t, Iode)
            if (
                self.op("ge", Iode[2], self.Itrip)
                or isclose(Iode[2], self.Itrip)
                or self.op("lt", dI[2], 0)
            ):  # peak current
    ...
    def set_dt_discharge(self):
        if self.dt_discharge == 0 and self.impulse:
            ...
            self.initalize(
                scenario=scenario, dt_discharge=dt_discharge, t_pulse=t_pulse
            )
    ...
    def packdata(self):
        ...
        self.Ivec = interp1d(
            ...
            self.data["Ivec"],
            axis=0,
            fill_value=(self.data["Ivec"][0], self.data["Ivec"][-1]),
            bounds_error=False,
        )
    ...
    def set_referance(self, ncoil=2):
        ...
        self.referance = {
            "t": np.copy(self.data["t"]),
            "Ic": np.copy(self.data["Ivec"][:, 0]),
        }
        self.referance["fun"] = interp1d(self.referance["t"], self.referance["Ic"])
        self.referance["to"] = np.linspace(
            self.referance["t"][0], self.referance["t"][-1], 350
        )
    ...
    def reduce(self):
        ...
        res = minimize(self.fit, xo, method="Nelder-Mead", options={"fatol": 0.01})
    ...
    def fit(self, x, plot=False):
        ...
        self.referance["Ifit"] = interp1d(
            ...
            self.data["Ivec"][:, 0],
            bounds_error=False,
            fill_value=(self.data["Ivec"][0, 0], self.data["Ivec"][-1, 0]),
        )(self.referance["to"])
        err = 1e-3 * np.sqrt(
            np.mean((self.referance["Ifit"] - self.referance["Ico"]) ** 2)
        )

        if plot:
            plt.figure()
            plt.plot(
                1e3 * self.referance["to"],
                1e-3 * self.referance["Ico"],
                label="referance",
            )
            plt.plot(
                1e3 * self.referance["to"],
                1e-3 * self.referance["Ifit"],
                "C3",
                label="fit",
            )
...
if __name__ == "__main__":
    ...
    ps.solve(Io=0, sign=-1, t_pulse=0, origin="peak", impulse=False, plot=True)
