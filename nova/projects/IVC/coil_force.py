...
class coil_force(pythonIO):
    def __init__(
        ...
        self.pl = read_plasma(
            "disruptions", Ip_scale=self.Ip_scale, read_txt=read_txt
        )  # load plasma
        self.tor = read_tor(
            "disruptions", Ip_scale=self.Ip_scale, read_txt=read_txt
        )  # load currents
        self.ps = power_supply(nturn=nturn, Ip_scale=self.Ip_scale, read_txt=read_txt)
    ...
    def load_file(self, scenario, **kwargs):
        ...
        if read_txt or not isfile(filepath + ".pk"):
            ...
            self.save_pickle(
                filepath, ["Ivs3_fun", "vs_geom", "pf", "ff", "xlim", "zlim"]
            )
        else:
    ...
    def add_vv_coils(self):
        ...
        if self.vessel:  # add vv and trs coils
            ...
            self.pf.add_coils(jacket_coil, subcoil=coilset["subcoil"], label="jacket")
            vv_coil = {
                name: coilset["coil"][name] for name in coilset["coil"] if "vv" in name
            }
            self.pf.add_coils(vv_coil, subcoil=coilset["subcoil"], label="vv")
            trs_coil = {
                name: coilset["coil"][name] for name in coilset["coil"] if "trs" in name
            }
            self.pf.add_coils(trs_coil, subcoil=coilset["subcoil"], label="trs")
            # remove DINA coils
            vv_remove = (
                [0, 1]
    ...
    def contour(self, plot=True, ax=None, **kwargs):
        self.psi = cc.get_coil_psi(
            self.x2d, self.z2d, self.pf.coilset["subcoil"], self.pf.coilset["plasma"]
        )
        self.sf = SF(
            eqdsk={
                "x": self.x,
        ...
        if plot:
            ...
            levels = self.sf.contour(boundary=True, Xnorm=True, ax=ax, **kwargs)
    ...
    def get_stress(self, Fn, Ft, name):
        ...
        PmPb = np.sqrt(
            (
                ...
                + dy * abs(Moment["x"]) / (2 * Ix)
            )
            ** 2
            + 3 * ((Stress["x"]) ** 2 + (Stress["y"]) ** 2)
        )
        Sm = self.allowable.get_limit(
            weld_category=2, load_category=1, load_type="primary", T=100
        )
    ...
    def get_field(self, nframe=None):
        ...
        for i, frame_index in enumerate(frames):
            ...
            for mode in B_data:
                ...
                for j, name in enumerate(self.pf.coilset["index"]["VS3"]["name"]):
                    coil = self.pf.coilset["coil"][name]
                    point = [coil["x"], coil["z"]]
    ...
    def movie(self, folder, nframe=None, mode="reference", discharge="DINA"):
        ...
        with writer.saving(fig, filename, 72):
            for frame_index in frames:
                plt.clf()
                self.plot_frame(frame_index=frame_index, mode=mode, levels=levels)
                writer.grab_frame()
                tick.tock()
    ...
    def plot_Fmax(self, nframe=100):
        ...
        for i, name in enumerate(coil_data):
            ...
            for j, (mode, color, width) in enumerate(
                ...
                for iax, k in enumerate([1, 0]):  # upper / lower VS coils
                    ...
                    Fmax[mode][i]["Fmag"][k] = vs3[mode]["Fmag"][frame_index, k]
                    for var in ["t", "I"]:
                        Fmax[mode][i][var][k] = vs3[mode][var][frame_index]

                    ax[iax].bar(
                        i,
                        1e-3 * Fmax[mode][i]["Fmag"][k],
                        color=color,
                        width=width,
                        label=mode,
                    )
        for i, k in enumerate([1, 0]):
            ...
            ax[i].text(
                im[index],
                1e-3 * Fm[index],
                "{:1.0f}".format(1e-3 * Fm[index]),
                weight="bold",
                va="bottom",
                ha="center",
            )
        h = []
        for mode, color in zip(["reference", "control", "error"], ["C1", "C2", "C3"]):
        ...
        for i, name in enumerate(coil_data):
            ...
            for mode, color, width in zip(modes, colors, widths):
                ax_I.bar(
                    i, 1e-3 * Imax[mode][i]["I"], color=color, width=width, label=mode
                )
        Im, im = [], []
        for mode in Imax:
        ...
        for x, label in zip(X, folders):
            ...
            plt.text(
                x, 60, label + " ", rotation=90, va="top", ha="center", color="white"
            )
        plt.despine()
        if len(modes) > 1:
            ax_I.legend(
        ...
        for index, file in enumerate(coil_data):
            ...
            for i, mode in enumerate(["reference", "control", "error"]):
                ax[i].plot(1e3 * vs3[mode]["t"], 1e-3 * vs3[mode]["I"], color=color)
        for i, (mode, fc) in enumerate(
            zip(["reference", "control", "error"], ["gray", "C0", "C3"])
        ):
        ...
        mode_index = np.nanargmax([max(abs(Imax[mode]["I"][:])) for mode in Imax])
        ...
        for i, (k, color_I) in enumerate(zip([1, 0], ["C6", "C7"])):
            name = self.pf.coilset["index"]["VS3"]["name"][k]
            for j, (color, va) in enumerate(zip(["C0", "C3"], ["bottom", "top"])):
                # MD then VDE
                ...
                ax[i].plot(
                    1e3 * vs3[mode]["t"], 1e-3 * vs3[mode]["Fmag"][:, k], color=color_I
                )
                txt = file + "\n"
                ...
                ax[i].text(1e3 * t_max + 25, 1e-3 * F_max, txt, va=va, ha="left")

                ax_I.plot(1e3 * vs3[mode]["t"], 1e-3 * vs3[mode]["I"][:], color=color_I)
                txt = file + "\n"
                txt += "$t=$" + "{:1.1f}ms, ".format(1e3 * t_max)
                ...
                ax_I.text(1e3 * t_max + 25, 1e-3 * I_max, txt, va=vaI, ha="left")
                print(name, file, frame_index_max, t_max, 1e-3 * I_max)
                print(
                    "Fmax",
                    1e-3 * vs3[mode]["Fx"][Fmax_index],
...
if __name__ == "__main__":
    force = coil_force(
        vessel=True, t_pulse=0.3, nturn=3, Ip_scale=15 / 15, read_txt=False
    )
    # force.load_file(0)
