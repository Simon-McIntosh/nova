...
for key in coil_data:
    for iax, k in enumerate([1, 0]):
        ax[iax].plot(1e3*coil_data[key]['t'],
                     1e-3*coil_data[key]['Fmag'][:, k])
...
for _ax, coil in zip(ax, ['upperVS', 'lowerVS']):
    ...
    _ax.text(0.95, 0.95, coil, transform=_ax.transAxes,
             va='top', ha='right', bbox=dict(facecolor='w', ec='gray', lw=1,
