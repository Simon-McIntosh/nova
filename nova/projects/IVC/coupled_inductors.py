...
class inductance:
    ...
    def constrain(self):  # apply and colapse constraints
        ...
        self.M = np.append(self.M[self.nd["dr"], :], self.M[self.nd["dc"], :], axis=0)
        self.M = np.append(self.M[:, self.nd["dr"]], self.M[:, self.nd["dc"]], axis=1)
        self.M = np.dot(np.dot(self.T.T, self.M), self.T)
        # turn resistance
        self.Rc = np.append(self.Rc[self.nd["dr"]], self.Rc[self.nd["dc"]], axis=0)
        self.Rc = np.dot(self.T.T, self.Rc)  # sum
        # turn current
        self.It = np.append(self.Io[self.nd["dr"]], self.Io[self.nd["dc"]], axis=0)
