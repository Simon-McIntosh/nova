    ...
    def get_discharge(self, code, eps=1e-5):
        ...
        for name in data:
            ...
            trim_index = (
                next(
                    i
                    for i, Id in enumerate(data[name]["coils"].loc[:, "Id"])
                    if Id < (1 - eps) * Io
                )
                - 1
            )
            data[name]["discharge"] = pd.DataFrame()
            data[name]["discharge"]["t"] = data[name]["coils"].loc[trim_index:, "t"]
            data[name]["discharge"]["t"] -= data[name]["coils"].loc[trim_index, "t"]
            data[name]["discharge"]["I"] = data[name]["coils"].loc[trim_index:, "Id"]
            data[name]["Id"] = interp1d(
                data[name]["discharge"]["t"],
                data[name]["discharge"]["I"],
                bounds_error=False,
                fill_value=(
                    data[name]["discharge"].iloc[0]["I"],
            ...
            data[name]["Io"], data[name]["tau"] = tc.nfit(
                2,
                plot=False,
                Io=Io,
            )[:2]
            data[name]["Ifit"] = np.zeros(len(data[name]["discharge"]["t"]))
            for Io, tau in zip(data[name]["Io"], data[name]["tau"]):
                data[name]["Ifit"] += Io * np.exp(-data[name]["discharge"]["t"] / tau)
