...
class elm_flux(pythonIO):
    ...
    def load_file(self, folder, plot=False, **kwargs):
        self.folder = folder
        self.reverse_current = kwargs.get("reverse_current", self.reverse_current)
    ...
    def calculate(self):
        self.names = np.unique(
            ["_".join(name.split("_")[:-1]) for name in self.coilset["coil"]]
        )
        ...
        for name in self.names:
            ...
            for n in [name, name_bg]:
                ...
                self.Vinterp[n] = interp1d(
                    self.t, self.V[n], fill_value=0, bounds_error=False
                )
