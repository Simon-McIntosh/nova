...
class midplane:
    ...
    def solve_field(self, point=(6.1929, 0.5014), n=300, plot=False, **kwargs):
        self.Ivs3_fun = self.ps.solve(**kwargs)  # solve power supply
        data = np.zeros(n, dtype=[("time", float), ("B", "2float"), ("Ivs3", float)])
        data["time"] = np.linspace(self.ps.data["t"][0], self.ps.data["t"][-1], n)
    ...
    def plot_timeseries(self, data):
        ...
        for simulation in data:
            self.plot_field(
                data[simulation], tmax=1.8, label=simulation.replace("_", " "), ax=ax
            )
...
if __name__ == "__main__":
    ...
    data["ex-vessel"] = mp.solve_field(
        vessel=True, invessel=False, Vo_factor=2.5, Ipulse=83.4e3, plot=True
    )
    data["in-vessel"] = mp.solve_field(
        vessel=True, invessel=True, Vo_factor=1, Ipulse=60e3, plot=True
    )
    data["no-vessel"] = mp.solve_field(
        vessel=False, invessel=True, Vo_factor=1.5, Ipulse=60e3, plot=True
    )
