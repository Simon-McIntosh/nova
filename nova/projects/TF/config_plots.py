...
profile = Profile(
    config["TF"], family="S", load=True, part="TF", nTF=nTF, obj="L", npoints=250
)
...
offset = (r1 - ro) / 2
rcl, zcl = geom.offset(
    demo.parts["TF_Coil"]["in"]["r"], demo.parts["TF_Coil"]["in"]["z"], offset
)
...
cage = coil_cage(
    nTF=nTF, rc=tf.rc, plasma={"config": config["eq"]}, coil={"r": rcl, "z": zcl}
)
