...
class ripple(object):
    ...
    def get_seperatrix(self, nplasma=80, alpha=1 - 1e-3, **kwargs):
        ...
        r, z = geom.clock(r, z)
        (self.plasma_loop[:, 0], self.plasma_loop[:, 2]) = geom.rzSLine(
            r, z, npoints=self.nplasma
        )
        self.plasma_length = geom.length(self.plasma_loop[:, 0], self.plasma_loop[:, 2])
    ...
    def set_TFcoil(self, cl, npoints=80, smooth=False):
        ...
        self.coil_loop[:, 0], self.coil_loop[:, 2] = geom.rzSLine(
            r, z, npoints=npoints
        )  # coil centerline
    ...
    def get_ripple(self):
        self.res = minimize_scalar(
            self.ripple_opp, method="bounded", bounds=[0, 1], options={"xatol": 0.1}
        )
    ...
    def plot_loops(self, scale=1.5):
        ...
        x = self.res["x"]
        pl.plot(self.plasma_interp["r"](x), self.plasma_interp["z"](x), ".", ms=10)
        pl.axis("equal")
        pl.axis("off")
