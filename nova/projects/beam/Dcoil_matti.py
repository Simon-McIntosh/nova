...
for m in range(len(theta)):
    while dSUM[m] >= 0.001:
        for n in range(len(nnn)):
            SUM[m, n] = (1j / nnn[n]) * (np.exp(-1j * nnn[n] * theta[m]) - 1) * (
                1 + np.exp(1j * nnn[n] * (theta[m] + np.pi))
            ) * (np.exp(1j * nnn[n] * np.pi / (2))) * 0.5 * (
                special.iv(nnn[n] - 1, k) + special.iv(nnn[n] + 1, k)
            ) + SUM[
                m, n - 1
            ]
...
mp.text(
    0,
    20,
    r"$z=r_{0}k\left(I_{1}(k)\theta+ \sum_{n=1}^\infty\frac{i}{n}[e^{-in\theta}-1][1+e^{in(\theta+\pi)}]e^{in\frac{\pi}{2}}\frac{I_{n-1}(k)+I_{n+1}(k)}{2} \right) $",
    fontsize=14,
)
mp.text(0, 15, r"$r=r_{0}e^{k\sin{\theta}}$", fontsize=14)
mp.text(0, 30, r"$k=\frac{1}{2}\ln{\frac{r_2}{r_1}}=%s$" % round(k, 3), fontsize=14)
