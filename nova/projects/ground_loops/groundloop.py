...
class GroundLoop(VDE):
    ...
    def build_loop(self):
        ...
        target = BiotFrame(
            additional=["link"],
            default=dict(nturn=self.nturn, dl=0.1, dt=0.1, segment="circle"),
        )
        ...
        self.loop.data.attrs["Lo"] = Biot(
            target, target, reduce=[True, True], turns=[True, True], columns=["Psi"]
        ).data.Psi.values[0]
    ...
    def filter_waveform(self, cutoff):
        ...
        time = np.linspace(
            self.loop.data._time[0], self.loop.data._time[-1], step_number
        )
        flux = scipy.interpolate.interp1d(self.loop.data._time, self.loop.data._flux)(
            time
        )
        ...
        self.loop.data["flux"] = xarray.DataArray(
            0.0, dims=["time"], coords=[self.loop.data.time]
        )
        self.loop.data["emf"] = xarray.DataArray(
            0.0, dims=["time"], coords=[self.loop.data.time]
        )
        self.loop.data["flux"] = ("time", scipy.signal.savgol_filter(flux, window, 1))
        # self.loop.data['flux'] = ('time', flux)
        self.loop.data["emf"] = ("time", -np.gradient(self.loop.data["flux"], time))
        self.loop.data["power"] = self.loop.data["emf"] ** 2 / self.loop.data.attrs["R"]

    def plot_waveform(self, axes=None):
        """Plot ground loop waveforms."""
        if axes is None:
            axes = plt.subplots(3, 1, sharex=True)[1]
        energy = np.trapz(self.loop.data["power"], self.loop.data["time"])
        label = self.folder.replace("_", " ") + f" {energy:1.1f}J"
        axes[0].plot(self.loop.data["time"], self.loop.data["flux"], label=label)
...
if __name__ == "__main__":
    ...
    nturn = (
        np.arcsin(
            np.linalg.norm([9.5302 - 9.9062, 2.7783 - 0.6483])
            / (2 * np.linalg.norm([9.905, 0.645]))
        )
        / np.pi
    )

    gl = GroundLoop(folder=folder, inner=ISS, outer=cryo, nturn=nturn, read_txt=False)
