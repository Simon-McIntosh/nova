...
mesh = {}
mesh["rz"] = fipy.CylindricalGrid2D(nr=nr, dr=dr, nz=nz, dz=dz, origin=origin)
...
rconv.setValue([mesh["xy"].faceCenters[0] ** -1, np.zeros(mesh["xy"].numberOfFaces)])
...
viewer = {}
viewer["f"] = fipy.Viewer(
    (f), datamin=0, datamax=1, axes=ax[0], colorbar=None, cmap=plt.cm.inferno
)
viewer["f"].plot()

eq["rz"].solve(var=crz)
viewer["rz"] = fipy.Viewer(
    (crz), datamin=0, datamax=1, axes=ax[1], colorbar=None, cmap=plt.cm.inferno
)
viewer["rz"].plot()
...
viewer["xz"] = fipy.Viewer(
    (cxy), datamin=0, datamax=1, axes=ax[2], colorbar=None, cmap=plt.cm.inferno
)
viewer["xz"].plot()


nznr = (mesh["rz"].numberOfHorizontalRows - 1, mesh["rz"].numberOfVerticalColumns - 1)
...
ax.plot(r1d, f_interp.ev(z_interp * np.ones(len(r1d)), r1d), "--", label="function")
ax.legend()
plt.despine()
ax.set_xlabel("$r$")
ax.set_ylabel("$c$")
