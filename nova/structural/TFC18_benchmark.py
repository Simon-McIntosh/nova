...
class ErrorProfile:
    ...
    scenario: str
    key: dict = field(
        default_factory=lambda: dict(scod="pcr100", SCOD="pcr100", F4E="f4e", MAG="mag")
    )
    mesh: pv.PolyData = field(init=False)
    ...
    def extract_cetnerline_keypoints(self):
        ...
        interp_z = scipy.interpolate.interp1d(coil.points[:, 2], coil["arc_length"])
        return dict(
            ILIS_top=interp_z(4),
    ...
    def vtk_file(self):
        """Return vtk filepath."""
        return os.path.join(
            root_dir, "data/Assembly/TFC18", self.dataset + f"_{self.scenario}.vtk"
        )
    ...
    def _f4e(self):
        ...
        if self.scenario != "EOB":
            raise IndexError(
                f"{self.scenario} not present " "in F4E dataset (only EOB)"
            )
    ...
    def plot(self, coordinate=1, clip_x=True, axes=None):
        ...
        for i in range(18):
            ...
            self.plot_coil(
                coil, coordinate=coordinate, axes=axes, label=label, color=colors[i]
            )
        label.plot()

    def plot_coil(
        self, coil, coordinate=0, axes=None, delta=True, label=None, color=None
    ):
        """Generate 2d plots."""
        if axes is None:
            axes = plt.gca()
        ...
        if delta:
            axes.plot(coil["abscissa"], 1e3 * coil["delta"][:, coordinate], color=color)
            if label:
                label.add(coil["coil_names"][0])
        else:
            axes.plot(
                coil["arc_length"], 1e3 * coil["displace_a"][:, coordinate], color=color
            )
            if label:
                label.add(coil["coil_names"][0])
            axes.plot(
    ...
    def shade(self, axes=None):
        ...
        ylim = axes.get_ylim()
        axes.fill_between(
            coil["arc_length"][index],
            ylim[1] * np.ones(n_shade),
            ylim[0] * np.ones(n_shade),
            color="gray",
            alpha=0.2,
            lw=0,
        )
    ...
    def rms(self):
        ...
        for i in range(18):
            ...
            rms[i] = np.sqrt(np.mean(np.linalg.norm(coil["delta"][:, :2], axis=1) ** 2))
    ...
    def bar(
        ...
        if text:
            ...
            axes.text(0.95, 0.95, text, transform=axes.transAxes, ha="right", va="top")
        plt.despine()
    ...
    def extract_coilset_delta(self, arc_length: float):
        ...
        for i in range(18):
            ...
            delta[i] = scipy.interpolate.interp1d(
                coil["arc_length"], coil["delta"], axis=0
            )(arc_length)
        return delta
    ...
    def plot_wave(self, axes, values, wavenumber=1, ncoil=18):
        ...
        axes.text(
            ncoil - 0.5, ifft[-1], label, va="center", color="C6", fontsize="small"
        )
    ...
    def plot_keypoints(self, labels=None):
        ...
        points = scipy.interpolate.interp1d(coil["arc_length"], coil.points, axis=0)
        ...
        for i, keypoint in enumerate(self.keypoints):
            ...
            plt.text(point[0], point[2], f" {labels[i]}", va="bottom", ha="left")
        plt.axis("equal")
        plt.axis("off")
    ...
    def error_frame(self):
        ...
        error = pandas.DataFrame(index=index, columns=[f"k{i}" for i in range(10)])

        for i, (keypoint, label) in enumerate(zip(self.keypoints, "cbdaef")):
            length = self.keypoints[keypoint]
            delta = self.extract_coilset_delta(length)
            for j, coord in enumerate(["r", "phi", "z"]):
                error.loc[(coord, label), :] = self.error_modes(1e3 * delta[:, j])
        return error
...
class BenchMark:
    ...
    def __post_init__(self):
        ...
        if isinstance(self.scenario, str):
            self.scenario = 2 * [self.scenario]
        self.profile = [
            ErrorProfile(self.case_a, self.scenario[0]),
    ...
    def bar(
        ...
        if axes is None:
            axes = plt.gca()
        self.profile[0].bar(
            keypoint, coordinate, axes=axes, label="mode", annotate=False, text=False
        )
        self.profile[1].bar(
            keypoint,
            coordinate,
    ...
    def bar_array(self, coordinate: int):
        ...
        text = "cbdaef"
        for i, (keypoint, axes) in enumerate(
            zip(self.profile[0].keypoints, axes_array.reshape(-1))
        ):
            self.bar(
                keypoint, coordinate, axes, legend=True, annotate=False, text=text[i]
            )
        ...
        axes_array[1][0].set_ylabel(rf"$\Delta\,{coordinate_label[coordinate]}$ mm")
    ...
    def case_labels(self):
        ...
        if self.scenario[0] != self.scenario[1]:  # include scenario
            labels = [
                f"{label} {self.profile[i].scenario}" for i, label in enumerate(labels)
            ]
        return labels
    ...
    def maximum_error(self):
        ...
        max_error = pandas.DataFrame(
            index=["coord", "point", "delta", "percent"],
        ...
        for mode in range(1, 10):
            ...
            max_error.loc["percent", f"k{mode}"] = 1e2 * (
                error.iloc[-1, col] / error.iloc[0, col]
            )
        return max_error
