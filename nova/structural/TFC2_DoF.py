...
class TFC2_DoF(AnsysDataDir):
    ...
    def plot_slice(self, loadcases=[6, 7, 8, 9, 10, 11], factor=750):
        """Plot dataset slice."""
        plotter = pv.Plotter(
            shape=(len(loadcases) + 1, self.n_datasets),
            border=False,
        ...
        for i, dataset in enumerate(self.datasets):
            for j, loadcase in enumerate(loadcases):
                plotter.subplot(j, i)
                warp = self.mesh.warp_by_vector(
                    f"{dataset}_disp-{loadcase}", factor=factor
                )
                plotter.add_mesh(
                    warp, scalars=f"{dataset}_vm-{loadcase}", scalar_bar_args=sargs
                )
    ...
    def load_xarray(self):
        ...
        self.data = xarray.Dataset(
            coords=dict(
                coil=list(self.reference_current),
        ...
        self.data["disp"] = (["trial", "points", "vector"], self.get_data("disp"))
...
class Simulator:
    ...
    def load_models(self):
        ...
        for model in ["linear", "kridge"]:
            for attr in ["disp", "vm"]:
                self.models[f"{model}_{attr}"] = Regressor(
                    self.design.data.copy(deep=True), model, attr
                )
    ...
    def plot_compare(self, trial, clim=None):
        ...
        for model in ["linear", "kridge"]:
            self.mesh[f"predict_{model}"] = self.models[f"{model}_vm"].predict(current)
