...
class AsBuilt:
    ...
    def read_ccl(self):
        """Read xls file."""
        self.ccl = pandas.read_excel(
            self.xls_file, sheet_name="CCL", header=[0, 1], index_col=[0, 1]
        )
        for step in self.ccl_steps:
            for coord in "xyz":
                self.ccl.loc[:, (step, f"d{coord}")] = (
                    self.ccl.loc[:, (step, coord)] - self.ccl.loc[:, ("Nominal", coord)]
                )
    ...
    def ccl_deltas(self):
        ...
        return {
            coil: self.ccl.loc[f"TF{coil:02d}", ("FAT", ["dx", "dy", "dz"])]
            for coil in coils
        }
