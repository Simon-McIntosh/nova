...
class CoilCage(DataDir, Plotter):
    ...
    def from_xarray(self, data: xarray.Dataset):
        ...
        label = [
            f"{coil:02d}" if clone == -1 else f"{coil:02d}<{clone:02d}"
    ...
    def add_scenario(self, data: xarray.Dataset, scenario: str, source="fit_delta"):
    ...
    def build_dataset(self):
        ...
        for label, weight in zip(["inboard", "equal", "outboard"], [-1, 0, 1]):
