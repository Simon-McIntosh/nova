...
class InnerLeg:
    ...
    def pick_points(self):
        ...
        self.points = []
        plotter = pv.Plotter()
        plotter.enable_point_picking(callback=self.store,
...
class Coil:
    ...
    def assert_wedge(self):
        ...
        assert np.isclose(
            np.pi - np.arccos(np.dot(self.ilis[0].norm, self.ilis[1].norm)),
            np.pi/9)
    ...
    def rotate_z(self, angle, case=True):
        ...
        for ilis in self.ilis:
            ilis.rotate_z(angle)
        self.origin = \
            Rotation.from_euler('Z', angle, degrees=True).apply(self.origin)
        return self
    ...
    def plot(self, plotter=None, show=True, case=True):
        ...
        plotter.add_mesh(pv.Line([*self.origin[:2], -5],
                                 [*self.origin[:2], 5]), color='w')
        plotter.add_axes()
        if show:
            plotter.show(window_size=(600, 600))
        return plotter
...
class Cage:
    ...
    def __post_init__(self):
        ...
        self.data = self.data.assign_coords(
            azimuth=('coil', [FiducialData.location.index(coil)*np.pi / 9
                              for coil in self.data.coil]))
        self.data = self.data.sortby('azimuth')

        self.data['ilis'] = (('coil', 'side', 'corner', 'coordinate'),
                             np.zeros((18, 2, 4, 3)))
        self.data['point'] = (('coil', 'side', 'coordinate'),
                              np.zeros((18, 2, 3)))
        self.data['normal'] = (('coil', 'side', 'coordinate'),
                               np.zeros((18, 2, 3)))
        for i in range(18):
            for j in range(2):
                self.data['ilis'][i, j] = self.coil.ilis[j].points
                self.data['point'][i, j] = \
                    self.coil.ilis[j].points.mean(axis=0)
                self.data['normal'][i, j] = self.coil.ilis[j].norm
            self.coil.rotate_z(20, case=False)
        self.data.attrs = dict(unit='m',
    ...
    def plot_corners(self, plotter=None, show=True):
        ...
        if plotter is None:
            plotter = pv.Plotter()
        plotter.add_mesh(pv.PolyData(self.data.ilis[:, 0].data.reshape(-1, 3)),
                         color='r')
        plotter.add_mesh(pv.PolyData(self.data.ilis[:, 1].data.reshape(-1, 3)),
                         color='b')
        if show:
            plotter.show(window_size=(600, 600))
        return plotter
...
if __name__ == '__main__':
    ...
    ilis_a = [[3.1102735026974, -0.544363429461, 4.3580624691689],
    ...
    ilis_b = [[2.2783856668339473, 0.3986945825098288, 4.7462638399745],
