...
class SS:  # structural solver
    ...
    def __init__(self, sf, pf, tf):
        ...
        self.inv.colocate(
            sf, n=1e3, expand=0.5, boundary="tf", centre=0, width=363 / (2 * np.pi)
        )
    ...
    def add_mat(self, ntrans=30):
        self.fe.add_mat(
            "nose",
            ["wp", "steel_forged"],  # high field TF
            [self.atec.winding_pack(), self.atec.case(0)],
        )
        self.fe.add_mat(
            "loop",
            ["wp", "steel_cast"],  # low field TF
    ...
    def connect_pf(self):
        for name in self.atec.PFsupport:  # PF coil connections
            ...
            self.fe.add_elements(
                n=[self.fe.nnd - 2, self.fe.nnd - 1], part_name=name, nmat=mat_name
            )
    ...
    def add_pf_load(self):
        ...
        for name in self.atec.PFsupport:  # PF coil connections
            ...
            self.fe.add_nodal_load(
                self.atec.PFsupport[name]["nd"], "fz", 1e6 * Fcoil["fz"] / self.tf.nTF
            )

    def gravity_support(self, nGS=7):
        yGS = np.linspace(self.atec.Gsupport["yfloor"], 0, nGS)
        zGS = np.linspace(
            self.atec.Gsupport["zfloor"], self.atec.Gsupport["zbase"], nGS
        )
        for ygs, zgs in zip(yGS, zGS):
            self.fe.add_nodes([self.atec.Gsupport["Xo"], ygs, zgs])
        yGS = np.linspace(0, -self.atec.Gsupport["yfloor"], nGS)
        zGS = np.linspace(
            self.atec.Gsupport["zbase"], self.atec.Gsupport["zfloor"], nGS
        )
        for ygs, zgs in zip(yGS[1:], zGS[1:]):
            self.fe.add_nodes([self.atec.Gsupport["Xo"], ygs, zgs])
        self.fe.add_elements(
            n=range(self.fe.nnd - 2 * nGS + 1, self.fe.nnd), part_name="GS", nmat="GS"
        )

        # couple GS to TF loop
        self.fe.add_cp(
            [self.atec.Gsupport["nd_tf"], self.fe.nnd - nGS], dof="nrx", rotate=False
        )
    ...
    def outer_intercoil_supports(self, nIC=3):
        for name in self.atec.OICsupport:
            ...
            Xm = geom.qrotate(
                self.fe.X[nd_tf[1]],
                -np.pi / self.tf.nTF,  # minus
                xo=[0, 0, 0],
                dx=[0, 0, 1],
            )
            Xp = geom.qrotate(
                self.fe.X[nd_tf[1]],
                np.pi / self.tf.nTF,  # plus
                xo=[0, 0, 0],
                dx=[0, 0, 1],
            )
            ...
            for i in range(3):
                Xrotate[:nIC, i] = np.linspace(
                    Xm[0, i], self.fe.X[nd_tf[1]][i], nIC + 1
                )[:-1]
                Xrotate[nIC:, i] = np.linspace(
                    self.fe.X[nd_tf[1]][i], Xp[0, i], nIC + 1
                )[1:]
            ...
            self.fe.add_elements(n=nodes, part_name=name, nmat=name, el_dy=el_dy)
            self.fe.add_cp([nodes[0], nodes[-1]], dof="fix", rotate=True, axis="z")
