...
class architect(object):
    ...
    def plot_connections(self, *args):
        ...
        geom.polyfill(
            self.loop["cs"]["x"], self.loop["cs"]["z"], color=0.4 * np.ones(3)
        )
    ...
    def draw_OIS(self, L, width, thickness, TFloop):
        ...
        xcl = np.append(xcl + dn[0] * thickness / 2, xcl[::-1] - dn[0] * thickness / 2)
        zcl = np.append(zcl + dn[2] * thickness / 2, zcl[::-1] - dn[2] * thickness / 2)
    ...
    def OIC_support(self, thickness=0.15):  # outer intercoil support
        ...
        for i, (L, width) in enumerate(zip([0.4, 0.66], [4.5, 3.75])):
            name = "ois{:d}".format(i)
            self.loop["OIS"][name] = {"L": L, "width": width, "thickness": thickness}
    ...
    def support_arm(L, coil, TFloop):
        dl = np.sqrt(
            (coil["x"] - TFloop["x"](L)) ** 2 + (coil["z"] - TFloop["z"](L)) ** 2
        )
        return dl
    ...
    def connect_loop(self, coil, loop, edge=0.15, hover=0.1, argmin=60):
        L = minimize_scalar(
            architect.support_arm, method="bounded", args=(coil, loop), bounds=[0, 1]
        ).x
        ...
        if ndir < argmin:  # limit absolute support angle
            nhat = np.array(
                [np.sign(nhat[0]), np.tan(argmin * np.pi / 180) * np.sign(nhat[1])]
            )
    ...
    def PF_support(self, n=5):
        ...
        for name in self.pf.index["PF"]["name"]:
            ...
            nodes, p, ndir, L = self.connect_loop(
                coil, self.tf_fun_offset["out"], edge=0.15, hover=0.1, argmin=35
            )
    ...
    def get_TF_nodes(self):
        ...
        for name in self.PFsupport:
            self.PFsupport[name]["nd_tf"] = np.argmin(
                (tf_x - self.PFsupport[name]["p"]["x"][-1]) ** 2
                + (tf_z - self.PFsupport[name]["p"]["z"][-1]) ** 2
            )
        self.Gsupport["nd_tf"] = np.argmin(
            (tf_x - self.Gsupport["Xo"]) ** 2 + (tf_z - self.Gsupport["zbase"]) ** 2
        )
        for name in self.OICsupport:
            ...
            for i, (x, z) in enumerate(zip(p["x"], p["z"])):
                self.OICsupport[name]["nd_tf"][i] = np.argmin(
                    (tf_x - x) ** 2 + (tf_z - z) ** 2
                )
    ...
    def G_support(self, Xo=13, dZo=-1, width=0.75):  # gravity support
        L = minimize_scalar(
            architect.GS_placement,
            method="bounded",
            args=(Xo - width / 2, self.tf_fun["out"]),
            bounds=[0, 0.5],
        ).x
        coil = {
            "x": self.tf_fun["out"]["x"](L) + width / 2,
            "z": self.tf_fun["out"]["z"](L) - width / 2,
            "dx": width,
            "dz": width,
        }
        nodes, p = self.connect_loop(
            coil, self.tf_fun_offset["out"], edge=0, hover=0, argmin=90
        )[:2]
        ...
        z = [
            [self.pf.coil[name]["z"] - self.pf.coil[name]["dz"] / 2]
        ...
        alpha = np.arctan(
            (Xo * np.tan(theta) - width)
            / (self.Gsupport["zbase"] - self.Gsupport["zfloor"] - width / 2)
        )
        pin2pin = np.sqrt(
            (Xo * np.tan(theta) - width) ** 2
            + (self.Gsupport["zbase"] - self.Gsupport["zfloor"] - width / 2) ** 2
        )

        self.Gsupport["alpha"] = alpha
        self.Gsupport["pin2pin"] = pin2pin
        self.Gsupport["spread"] = pin2pin * np.sin(alpha) + 2 * width
        self.Gsupport["zground"] = (
    ...
    def adjust_TFnode(self, x, z):
        ...
        i = np.argmin(
            (self.tf.p["cl_fe"]["x"] - x) ** 2 + (self.tf.p["cl_fe"]["z"] - z) ** 2
        )
        dl = (
            np.sqrt(
                (self.tf.p["cl_fe"]["x"][i + 1] - self.tf.p["cl_fe"]["x"][i - 1]) ** 2
                + (self.tf.p["cl_fe"]["z"][i + 1] - self.tf.p["cl_fe"]["z"][i - 1]) ** 2
            )
            / 2
        )
        dx = np.sqrt(
            (self.tf.p["cl_fe"]["x"][i] - x) ** 2
        ...
        if dx > 0.5 * dl:  # add node (0.2*)
            ...
            Li = minimize_scalar(
                ...
                args=(
                    self.tf_fun["out"],
                    (self.tf.p["cl_fe"]["x"][i], self.tf.p["cl_fe"]["z"][i]),
                ),
                bounds=[0, 1],
            ).x
    ...
    def initalise_cs(self):
        ...
        self.cs["wp"] = {
            "d": self.tf.section["winding_pack"]["depth"],
    ...
    def tf_cs(self, plot=False):
        ...
        self.loop["case_in"] = {
            "z": np.array(
                [
                    -(case["hf_out"] + wp["w"] / 2),
                    rsep - (wp["xo"] - wp["w"] / 2 - case["hf_in"]),
                    wp["w"] / 2 + case["hf_in"],
                ]
            ),
            "y": np.array(
                [ynose, wp["d"] / 2 + case["side"], wp["d"] / 2 + case["side"]]
            ),
        }
        self.loop["case_out"] = {
            "z": np.array(
                [
                    -wp["w"] / 2 - case["lf_out"],
                    np.mean([case["lf_out"], case["lf_in"]]),
                    wp["w"] / 2 + case["lf_in"],
                ]
            ),
    ...
    def plot_tf_section(self):
        ...
        plt.axis("off")
        plt.xlim([-1, 0.5])
        plt.plot(self.loop["case_in"]["pnt"][0], self.loop["case_in"]["pnt"][1])
        geom.polyfill(
            self.loop["case_in"]["pnt"][0], self.loop["case_in"]["pnt"][1], color="C0"
        )
        geom.polyfill(self.loop["wp"]["pnt"][0], self.loop["wp"]["pnt"][1], color="C1")
        plt.sca(ax[1])
        plt.axis("equal")
        plt.axis("off")
        geom.polyfill(
            self.loop["case_out"]["pnt"][0], self.loop["case_out"]["pnt"][1], color="C0"
        )
        geom.polyfill(self.loop["wp"]["pnt"][0], self.loop["wp"]["pnt"][1], color="C1")
    ...
    def trans(self, frac):  # set transitional points
        ...
        for x in ["y", "z"]:
            self.loop["case"][x] = self.loop["case_in"][x] + frac * (
                self.loop["case_out"][x] - self.loop["case_in"][x]
            )
        self.get_pnt("case")
    ...
    def case(self, frac=0, plot=False, centroid=True):
        ...
        if frac < 1:  # inner or transitional
            ...
            sm.add_shape(
                "rect", b=y[0] - y[-1], h=z[1] - z[0], dz=z[1] - (z[1] - z[0]) / 2
            )
            if z[2] > z[1]:
                sm.add_shape(
                    "rect", b=y[1] - y[-2], h=z[2] - z[1], dz=z[1] + (z[2] - z[1]) / 2
                )
            else:
                sm.remove_shape(
                    "rect", b=y[1] - y[-2], h=z[1] - z[2], dz=z[1] + (z[2] - z[1]) / 2
                )
        else:
            sm.add_shape("rect", b=y[1] - y[2], h=z[1] - z[0], dz=np.mean([z[0], z[1]]))
    ...
    def plot_transition(self):
        ...
        for i, (fact, txt) in enumerate(
            zip([0, 0.5, 1], ["inboard", "transition", "outboard"])
        ):
            ax = plt.subplot(gs[i])
            ax.axis("equal")
            ax.axis("off")
