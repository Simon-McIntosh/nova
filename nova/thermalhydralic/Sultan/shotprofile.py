...
@dataclass
class ShotData:
    """Extract point data from ShotProfile.data."""
...
class ShotProfile:
    ...
    _shotdata: ShotData = field(init=False, repr=False)

    def __post_init__(self):
        """Build data pipeline."""
        self.reload.__init__(rawdata=True, lowpassdata=True,
                             heatindex=True, shotdata=True,
                             response=True, waveform=True)
    ...
    def sultandata(self):
        ...
        if self.instance.reload.sultandata:
            ...
            self._reload()
            self.instance.reload.sultandata = False
    ...
    @property
    def shotdata(self):
        """Return shotdata, read-only."""
        self._reload()
        if self.reload.shotdata:
            self._shotdata = ShotData(self.lowpassdata, self.heatindex.start,
                                      self.offset, self.normalize)
            self.reload.shotdata = False
