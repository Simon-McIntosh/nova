...
def _sort_monotonic(self, index):
    points = [
        (-np.log10(frequency), -np.log10(value))
        for frequency, value in zip(
            self.coefficents["frequency"][index], self.coefficents["dcgain"][index]
        )
    ]
...
def plot(self, Be, dcgain_limit=1e6):
    ...
    frequency = np.logspace(np.log10(_frequency[0]), np.log10(_frequency[-1]), 40)
    gain = scipy.interpolate.interp1d(np.log10(_frequency), _gain)(np.log10(frequency))
    ...
    def model_error(x):
        error = np.linalg.norm(np.log10(model_gain(x)) - np.log10(gain))
    ...
    lower_bounds = np.append(-1000 * frequency[-1] * np.ones(nzero + npole), 1e-8)
    ...
    vector = opt.optimize(
        np.append(-np.ones(nzero + npole), np.min([gain[0], upper_bounds[-1]]))
    )
    print(vector)
    ...
    plt.plot(frequency, model_gain(vector), label=f"limit {dcgain_limit:1.0e}")
    # print(poles)
