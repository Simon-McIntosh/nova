...
class SampleData(PandasHDF):
    ...
    def _read_data(self):
        columns = []
        for label in ["ICS_FL", "ICS_TS", "ICS_TC", "ICS_PT", "FAC_IC_CT", "ICS_FCT"]:
            columns.extend(self.nakadata.columns(label))
        nakadata = self.nakadata.data.loc[:, columns]
        labels = {column: "_".join(column.split("_")[1:]) for column in columns}
        nakadata.rename(columns=labels, inplace=True)
        thermometers = [
            label
            for label in nakadata.columns
            if label[0] == "T" and "STR" not in label
        ]
        position = {
            "IN": -2.25 - 0.9285,
            "08": -2.25 - (0.9285 - 0.646),
            "07": -2.25,
            "06": -1.5,
            "05": -0.75,
            "04": 0.75,
        ...
        for label in position:
            ...
            thermometer_label = next(meter for meter in thermometers if label in meter)
            data[thermometer_label.replace(label, label.lower())] = nakadata.loc[
                :, thermometer_label
            ]
