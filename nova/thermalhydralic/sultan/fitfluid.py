...
class FitFluid:
    ...
    def model_update(self, vector, grad):
        ...
        if self.verbose:
            ...
            sys.stdout.flush()
        if len(grad) > 0:
            grad[:] = scipy.optimize.approx_fprime(vector, self.model_error, 1e-6)
        return err

    def _set_nlopt(self, algorithm):
        opt = nlopt.opt(getattr(nlopt, algorithm), self.fluid.model.parameter_number)
    ...
    def optimize(self, waveform_data: pandas.DataFrame, optimizer="nlopt"):
        ...
        if optimizer == "scipy":
            res = scipy.optimize.minimize(self.model_error, self.fluid.model.vector)
