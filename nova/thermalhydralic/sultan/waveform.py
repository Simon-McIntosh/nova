...
class WaveForm:
    ...
    def _extract(self):
        ...
        if self.profile.normalize:
            ...
            field_amplitude = self.profile.sample.sourcedata.excitation_field
            fieldrate_amplitude = self.profile.sample.sourcedata.excitation_field_rate
        # build profiles
        ...
        self._data = pandas.DataFrame(
            np.array(
                [timeseries[0], field, fieldsq, fieldrate, fieldratesq, timeseries[1]]
            ).T,
            columns=["time", "field", "fieldsq", "fieldrate", "fieldratesq", "output"],
        )
    ...
    def timeseries(self, threshold=None, pulse=None, input_variable="fieldratesq"):
        """
