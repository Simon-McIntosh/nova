...
class pythonIO:
        ...
        with nc4.Dataset(filepath + ".nc", "w", format="NETCDF4") as dataset:
            ...
            for i, attribute in enumerate(attributes):
                ...
                if isinstance(dataframe.columns, MultiIndex):
                    dataframe.columns = ["|".join(c) for c in dataframe.columns.values]
                    attribute += ".mi"
                # encode columns
    ...
    def load_netCDF(self, filepath):
        ...
        for attribute in attributes:
            ...
            # decode columns
            dataframe.columns = self.netCDF_columns(dataframe.columns, decode=True)
            if attribute[-3:] == ".mi":  # re-create multi-index
                dataframe.columns = MultiIndex.from_tuples(
                    [(c.split("|")) for c in dataframe.columns], names=("name", "unit")
                )
        ...
        while True:
            ...
            try:
                if string:
                    ...
                    fr.append(
                        [
                            float(line[ncol_skip + i].replace("D", "E"))
...
def qsub(script, jobname="analysis", t=1, freiahost=1, verbose=True, Code="Nova"):
    p = Popen(
        split("ssh -T freia{:03.0f}.hpc.l".format(freiahost)),
        stdin=PIPE,
        stdout=PIPE,
        stderr=PIPE,
    )
...
class PATH(object):  # file paths
    ...
    def new(self, task):
        ...
        if task == 0:
            ...
            self.job = self.make_dir(
                self.data + "/" + self.date_str + "." + self.jobname
            )
            self.job = self.rep_dir(
                self.data
                + "/"
                + self.date_str
                + "."
...
class SET_PATH(object):  # file paths
    def __init__(self, jobname, **kw):
        ...
        self.folder = self.check_dir(
            self.data + "/" + self.date_str + "." + jobname
        )  # job
    ...
    def go(self, task):
        self.job = self.check_dir(
            self.data + "/" + self.date_str + "." + self.jobname + "/task." + str(task)
        )  # job
