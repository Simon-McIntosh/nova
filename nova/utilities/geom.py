...
def rdp_extract(x, y, eps=1e-2, dx=None, dx_window=None, scale=None, plot=False):
    """
    ...
    mask = rdp(np.vstack((xo / scale, yo)).T, epsilon=epsilon, return_mask=True).T
...
def trapz2d(z, x=None, y=None, dx=1.0, dy=1.0):
...
def Kcalc(R, col):  # K coefficents for rectangular beam torsion
    ...
    if col == 0:
        k = np.array([0.675, 0.759, 0.848, 0.93, 0.968, 0.985, 0.997, 0.999, 1, 1, 1])
    elif col == 1:
        k = np.array(
            [
                0.1406,
...
def poly_inloop(loop, point, plot=False, ax=None):
    ...
    if poly.intersects(points):
        ...
        if len(multi_point) == 2:
            ...
            interior_points = {
                "x": [point[0] for point in multi_point],
...
def polyline(loop, point, plot=False):
    ...
    poly = Polygon(np.array([loop["x"], loop["z"]]).T)
    points = np.array(
        [[point["x"][0], point["z"][0]], [point["x"][-1], point["z"][-1]]]
    )
...
def vector_length(X, norm=True):
    L = np.append(
        ...
        np.cumsum(
            np.sqrt(
                np.diff(X[:, 0]) ** 2 + np.diff(X[:, 1]) ** 2 + np.diff(X[:, 2]) ** 2
            )
        ),
    )
    if norm:
...
def lowpass(x, dt, dt_window=1, polyorder=3):
    ...
    if nwindow % 2 == 0:
        nwindow += 1
    x_lp = savgol_filter(x, nwindow, polyorder=polyorder, mode="mirror")  # lowpass
    return x_lp
...
def vector_lowpass(X, window_length=5, ployorder=3):
    ...
    Xf = np.zeros((m, n))
    for i in range(n):
        Xf[:, i] = savgol_filter(X[:, i], window_length, ployorder, mode="mirror")
    return Xf
...
class Loop(object):
    ...
    def fill(
        ...
        if isinstance(dt, list):
            dt = self.blend(dt, ref_o=ref_o, dref=dref, referance=referance, gap=gap)
        ...
        if plot:
            polyparrot(
                {"x": Xin, "z": Zin}, {"x": Xout, "z": Zout}, color=color, alpha=1
            )  # fill
        return Xout, Zout
        ...
        if plot:
            ...
            for i in np.arange(Lindex[0], Lindex[1] - 1):
                ...
                if flag == 0 and label is not None:
                    ...
                    plt.fill(
                        Rfill, Zfill, facecolor=color, alpha=alpha, edgecolor="none"
                    )
...
class String(object):
    ...
    def space(self, verbose=False, **kwargs):
        ...
        if verbose:
            ...
            print(
                "minimum pannel length: {:1.2f}, set: {:1.2f}".format(
                    np.min(delta_x), self.dx_min
                )
            )
            print(
                "maximum pannel length: {:1.2f}, set: {:1.2f}".format(
                    np.max(delta_x), self.dx_max
                )
            )
            print(
                "points input: {}, simplified: {}\n".format(
                    len(self.P), len(self.points)
                )
            )
    ...
    def plot(self, projection="2D", aspect=1):
        ...
        if projection == "3D":
            ...
            ax.plot(self.points[:, 0], self.points[:, 1], self.points[:, 2], "o-")
