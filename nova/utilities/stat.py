...
class histopeaks:  # extract peaks from discrete PDF data
    ...
    def plot_timeseries(self, **kwargs):
        ...
        for peak in self.peaks:
            ...
            ax[0].plot(
                self.time, 1e-6 * center * np.ones(self.nsum), "--", color="gray"
            )
            ax[0].fill_between(
                self.time,
        ...
        for i, ind in enumerate(self.mode_imax):
            ...
            if i == 0:
                ax[1].plot(
                    [self.time[ind[0]], self.time[ind[1]]],
    ...
    def count_peaks(self, nx):
        ...
        while True:
            if valley:
                i_rise = next(
                    (i + i_fall for i, n in enumerate(self.nhist[i_fall:]) if n > nx),
                    None,
                )
                valley = False
            else:
                i_fall = next(
                    (i + i_rise for i, n in enumerate(self.nhist[i_rise:]) if n < nx),
                    None,
                )
                valley = True
                if i_fall is not None:
                    ...
                    mean = np.sum(
                        self.bins[i_rise:i_fall] * self.nhist[i_rise:i_fall]
    ...
    def get_peaks(self, nlim=None, plot=False):
        ...
        self.peaks = np.zeros(
            nmax,
            dtype=[("centre", float), ("width", float), ("nmax", int), ("npeak", int)],
        )
    ...
    def plot_peaks(self):
        ...
        plt.plot(self.bins, self.nx * np.ones(len(self.bins)), "--", color="gray")
        plt.text(
            self.bins[-1],
            self.nx,
            "Nclip {:1.0f}".format(self.nx),
            ha="left",
            va="bottom",
        )
        for i, (centre, width, nmax, npeak) in enumerate(
            zip(
                self.peaks["centre"],
                self.peaks["width"],
                self.peaks["nmax"],
            ...
            for x in width / 2 * np.array([-1, 1]):
                plt.plot(
                    (centre + x) * np.ones(2), np.array([0, nmax]), "--", color="C1"
                )
        plt.xlabel("$x$")
        plt.ylabel("$N(x)$")
        plt.despine()
