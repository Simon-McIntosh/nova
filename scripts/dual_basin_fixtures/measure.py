...
def measure() -> tuple[dict[str, object], dict[str, object]]:
    ...
    report = {
        ...
        "classification": {
            ...
            "genuinely_diverted_fixture_in_bank": False,
            "diverted_fixture_action": (
                "construct and independently bank a state with a finite in-domain "
                "X-point whose flux, rather than wall contact, binds the separatrix"
            ),
            "two_class_acceptance_ready": False,
            "acceptance_implication": (
                "the bank can test two distinct fixed points and same-class branch "
                "handling, but cannot demonstrate limited/diverted recovery; "
                "relabeling either stored receipt would not supply the missing "
                "saddle evidence"
            ),
        },
    }
...
def draw(plot_data: dict[str, object]) -> Path:
    ...
    figure, axes = plt.subplots(1, 2, figsize=(9.2, 4.1), sharex=True, sharey=True)
    panels = (
...
def write_markdown(report: dict[str, object], figure: Path) -> Path:
    ...
    figure_link = (
        "![Fine-grid root topology, including the out-of-domain "
        "coordinate-axis saddle](topology-classification.png)"
    )
    table_header = (
        "| Resolution | Root | Class | Finite X-points | Binding wall contact "
