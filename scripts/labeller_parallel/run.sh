...
WRAP="export TMPDIR=/tmp JAX_PLATFORMS=cuda,cpu JAX_ENABLE_COMPILATION_CACHE=true XLA_PYTHON_CLIENT_PREALLOCATE=false PYTHONPATH='${ROOT}'; '${PYTHON}' '${DRIVER}' --output '${OUTPUT_ROOT}' --devices 1 --batch-per-device '${BATCH_PER_DEVICE}' --host-workers 3 --max-slices 4 --condition-on-guard-failure --replace"
COMMAND=(
  sbatch --parsable
