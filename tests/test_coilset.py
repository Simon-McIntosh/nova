...
def test_subframe_turn_number():
    coilset = CoilSet()
    coilset.coil.insert(1.75, 0.5, 2.5, 2.5, scale=1, delta=-4, nturn=22.2)
...
def test_circular_cross_section():
    coilset = CoilSet()
    coilset.coil.insert(1.75, 0.5, 2.5, 2.5, section="disc", turn="r", delta=-4)
    assert np.isclose(np.pi * 2.5**2 / 4, coilset.subframe.area.sum(), rtol=5e-3)


def test_rectangular_cross_section():
    coilset = CoilSet()
    coilset.coil.insert(1.75, 0.5, 2.5, 1.5, section="rectangle", scale=1, delta=-6)
...
def test_shell():
    ...
    coilset.shell.insert(
        [4, 6, 7, 9, 9.5, 6], [1, 1, 2, 1.5, -1, -1.5], 1, 0.1, delta=-1, label="vvin"
    )
...
def test_aspect_horizontal():
    coilset = CoilSet()
    coilset.coil.insert(1, 1, 0.75, 0.5, delta=-9, section="r", turn="s", fill=True)
    assert np.isclose(coilset.subframe.area.sum(), 0.75 * 0.5)


def test_aspect_vertical():
    coilset = CoilSet()
    coilset.coil.insert(1, 1, 0.5, 0.75, delta=-9, section="r", turn="s", fill=True)
    assert np.isclose(coilset.subframe.area.sum(), 0.75 * 0.5)
...
def test_tile_hex():
    coilset = CoilSet()
    coilset.coil.insert(
        1, 1, 0.5, 0.75, delta=-4, section="dsk", nturn=9, turn="hex", tile=True
    )
...
def test_plasma_single():
    ...
    coilset.firstwall.insert(
        [[1, 2, 2, 1.5, 1, 1], [1, 1, 2, 2.5, 1.5, 1]], turn="r", tile=False
    )
...
def test_plasma_hex():
    ...
    coilset.firstwall.insert(
        [[1, 2, 2, 1.5, 1, 1], [1, 1, 2, 2.5, 1.5, 1]], turn="hex", tile=True
    )
    assert sum([section == "hexagon" for section in coilset.subframe.section]) == 2
...
def test_store_load_poly():
    ...
    coilset.coil.insert(10, 0.5, 0.95, 0.95, section="hex", turn="r", nturn=-0.8)
