...
def test_array_ndarray():
    dataarray = DataArray({"x": [3, 2], "z": 0}, Required=["x", "z"], Array=["x"])
...
def test_get_loc_plasma():
    dataarray = DataArray(dict(x=range(3), plasma=True), Required=["x"], Array=["x"])
    assert dataarray.loc["plasma", "x"].tolist() == [0, 1, 2]


def test_get_loc_plasma_subset():
    dataarray = DataArray(
        dict(x=range(3), plasma=[True, False, True]), Required=["x"], Array=["x"]
    )
...
def test_get_loc_part():
    dataarray = DataArray(
        dict(x=range(5), part=["a", "b", "PF", "PF", "TF"]), Required=["x"], Array=["x"]
    )
...
def test_set_loc_part():
    dataarray = DataArray(
        dict(x=range(5), part=["a", "a", "PF", "PF", "TF"]), Required=["x"], Array=["x"]
    )
