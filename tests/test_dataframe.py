...
def test_columns_extend_additional():
    dataframe = DataFrame(Required=["x", "z"], Additional=[], columns=["x", "z", "rms"])
...
def test_data_init_additional():
    ...
    dataframe = DataFrame(
        data, metadata={"Required": ["x", "z"], "Additional": ["rms"]}
    )
...
def test_warn_new_attribute():
    dataframe = DataFrame({"x": [3, 4], "z": 0}, Required=["x", "z"], Subspace=[])
