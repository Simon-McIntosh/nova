...
class testbeam(finiteframe):
    ...
    def extract_model(self, i=2):
        if self.name == "axial tip point load":
            ...
            v = qrotate(self.part["beam"]["D"], theta=-self.theta, dx="y")[
                :, i
            ]  # deflection
    ...
    def check(self):
        for attribute in self.analytic:
            ...
            assert np.allclose(
                self.analytic[attribute], self.model[attribute], atol=atol
            )
...
def test_simple(plot=False):
    ...
    # analytic solution
    tb.analytic["v"] = (
        tb.w * tb.x / (24 * tb.EI) * (tb.L**3 - 2 * tb.L * tb.x**2 + tb.x**3)
    )
...
def test_cantilever(plot=False):
    ...
    # analytic solution
    tb.analytic["v"] = (
        tb.w * tb.x**2 / (24 * tb.EI) * (6 * tb.L**2 - 4 * tb.L * tb.x + tb.x**2)
    )
...
def test_pin_fix(plot=False):
    ...
    # analytic solution
    tb.analytic["v"] = (
        tb.w * tb.x / (48 * tb.EI) * (tb.L**3 - 3 * tb.L * tb.x**2 + 2 * tb.x**3)
    )
