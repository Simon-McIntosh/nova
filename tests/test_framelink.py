...
def test_data_It_Ic_unset_dict():
    framelink = FrameLink({"x": [1, 2], "It": 5, "nturn": 2.5}, Required=["x"])
    assert framelink.Ic.to_list() == [2, 2]


def test_data_It_Ic_set_dict():
    framelink = FrameLink(
        {"x": [1, 2], "It": 5, "Ic": 10, "nturn": 2.5}, Required=["x"]
    )
    assert framelink.Ic.to_list() == [10, 10]
    assert framelink.It.to_list() == [25, 25]
...
def test_frame_columns_multipoint():
    ...
    assert list(framelink.columns) == ["x", "z", "link", "factor", "ref", "subref"]
...
def test_columns():
    framelink = FrameLink(metadata={"Required": ["x", "z"], "Additional": ["rms"]})
...
def test_named_multi_insert():
    ...
    assert framelink.index.to_list() == ["PF4", "PF5", "CS11", "CS12", "PF6", "PF7"]
