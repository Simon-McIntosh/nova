...
def test_field(itime):
    ...
    field_index = [i for i, name in enumerate(benchmark.field.coil_name)
                   if name in benchmark.profile.data.coil_name and
                   benchmark.profile['b_field_max_timed'].data[i] != 0]
    profile_index = [i for i, name in
...
def test_force(itime):
    benchmark.itime = itime
    force_index = [i for i, name in enumerate(benchmark.force.coil_name)
