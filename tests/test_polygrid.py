...
def test_poly_buffer():
    polygrid = PolyGrid({"o": [6, 3, 0.4, 0.2]}, delta=-5, turn="sk", scale=1)
    assert np.sum([section == "skin" for section in polygrid.frame.section]) == 5


def test_square_effective_nfilament():
    polygrid = PolyGrid({"r": [6, 3, 2.5, 2.5]}, delta=-25, turn="sq", tile=False)
    assert np.isclose(-polygrid.delta, polygrid.polyfilaments)


def test_square_effective_nfilament_tile():
    polygrid = PolyGrid({"r": [6, 3, 2.5, 2.5]}, delta=-25, turn="sq", tile=True)
    assert np.isclose(-polygrid.delta, polygrid.polyfilaments)


def test_rectangle_effective_nfilament():
    polygrid = PolyGrid({"r": [6, 3, 2.5, 2.5]}, delta=-25, turn="r", tile=False)
    assert np.isclose(-polygrid.delta, polygrid.polyfilaments)


def test_rectangle_effective_nfilament_tile():
    polygrid = PolyGrid({"r": [6, 3, 2.5, 2.5]}, delta=-25, turn="r", tile=True)
    assert np.isclose(-polygrid.delta, polygrid.polyfilaments)


def test_disc_effective_nfilament():
    polygrid = PolyGrid({"r": [6, 3, 2.5, 2.5]}, delta=-25, turn="o", tile=False)
    assert np.isclose(-polygrid.delta, polygrid.polyfilaments, 1e-3)


def test_disc_effective_nfilament_tile():
    polygrid = PolyGrid({"r": [6, 3, 2.5, 2.5]}, delta=-69, turn="o", tile=True)
    assert np.isclose(-polygrid.delta, polygrid.polyfilaments, 1e-1)


def test_hexagon_effective_nfilament():
    polygrid = PolyGrid(
        {"r": [6, 3, 2.5, 2.5]}, delta=-25, turn="hx", tile=False, fill=True
    )
    assert np.isclose(-polygrid.delta, polygrid.polyfilaments, 1e-3)


def test_hexagon_effective_nfilament_tile():
    polygrid = PolyGrid({"r": [6, 3, 2.5, 2.5]}, delta=-49, turn="hex", tile=True)
