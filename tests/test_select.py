...
def test_plasma_subspace():
    framespace = FrameSpace(
        Required=["x"], Available=["active", "link"], Array=["plasma"]
    )
...
def test_coil():
    framespace = FrameSpace(Required=["x"], Available=["active"], Array=["coil"])
...
def test_select_subspace():
    ...
    assert np.array(
        [
            ...
            for attr in framespace.select.avalible
            if attr not in framespace.select.superspace
        ]
    ).all()
