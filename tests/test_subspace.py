...
from nova.frame.error import ColumnError, SpaceKeyError, SubSpaceKeyError
from nova.frame.framespace import FrameSpace


def test_init():
    framespace = FrameSpace(
        link=True, required=["x", "z", "dl", "dt"], metadata={"additional": ["link"]}
    )
...
def test_loc_slice():
    framespace = FrameSpace(
        Required=["x", "z"], Subspace=["Ic"], label="Coil", offset=15
    )
...
def test_set_It_array():
    framespace = FrameSpace(
        Required=["x", "z"], Subspace=["Ic", "It"], Array=["Ic", "nturn", "It"]
    )
    framespace.insert(0.5, [6, 8.3], nturn=0.5)
