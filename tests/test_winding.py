...
def test_a_swept_skin_keeps_its_circular_boundaries():
    """A skin is a disc with a disc removed, and used to be relabelled a box.

    The relabelling made its area the SQUARE annulus's -- 4/pi too large -- and
    handed every element a square where the section is round.  Both boundaries now
    come off the section itself, so the two members are 64-gons.
    """
